...
description: Built-in Tools in the Sunfish ToolBar for Blazor
slug: toolbar-built-in-tools
tags: sunfish,blazor,toolbar,builtin,tools,button,buttons
published: True
position: 1
components: ["toolbar"]
---
...
The Sunfish ToolBar for Blazor allows you to use built-in buttons and button groups or to add a [custom tool](slug:toolbar-templated-item). The available built-in tools are:
...
You can add multiple buttons to the Sunfish Toolbar. To do that you should add the `<ToolBarButton>` to the `<SunfishToolBar>`.
...
| `Icon` | `object` | Adds a font icon to the button. You can find more information on adding a font icon to a Sunfish Component in [Sunfish Font and Svg Icons article](slug:common-features-icons#icon-namespaces). |
...
>caption The Sunfish ToolBar with ToolBarButtons

![Blazor Toolbar Toolbarbutton Example](images/toolbar-toolbarbutton-example.png)

````RAZOR
@*This example shows the SunfishToolBar with ToolBarButtons and their features*@
...
You can add multiple toggle  buttons to the Sunfish Toolbar. To do that you should add the `<ToolBarToggleButton>` to the `<SunfishToolBar>`.
...
>caption The Sunfish ToolBar with ToolBarToggleButtons

![Blazor Toolbar Togglebutton Example](images/toolbar-togglebutton-example.png)

````RAZOR
@*This example shows the SunfishToolBar with ToolBarToggleButton and its available features*@
...
You can add one or more group of buttons to the Toolbar. To do that you should add the `<ToolBarButtonGroup>` to the `<SunfishToolBar>`. In the button group you can place either the `ToolBarButton` or the `ToolBarToggleButton`.
...
>caption The Sunfish ToolBar with grouped buttons

![Blazor Toolbar Grouped Buttons](images/toolbar-grouped-buttons.png)

````RAZOR
@*This example shows the SunfishToolBar with grouped ToolBarButtons*@
