...
tags: sunfish,blazor,dialog,overview
published: True
position: 0
components: ["dialog"]
---
...
1. Add the `SunfishDialog` tag to a Razor file.
...
5. (optional) Configure the [`DialogButtons`](slug:dialog-action-buttons) inside the `SunfishDialog` tag.

>caption A basic configuration of the Sunfish Dialog.
...
The Blazor Dialog provides various parameters to configure the component. Also check the [Dialog public API](slug:Sunfish.Blazor.Components.SunfishDialog).
...
| `ThemeColor` | `string` | A predefined color scheme for the Dialog, especially the titlebar. Use the available members of the static class [`ThemeConstants.Dialog.ThemeColor`](slug:Sunfish.Blazor.ThemeConstants.Dialog.ThemeColor). |
...
<SunfishButton  OnClick="OpenDialog">Open Dialog</SunfishButton>

<SunfishDialog @ref="DialogRef" @bind-Visible="_dialogVisible">
    <DialogContent>
        <p role="status">Current count: @_currentCount</p>
    </DialogContent>
    <DialogButtons>
        <SunfishButton OnClick="IncrementCount">Increment Count</SunfishButton>
        <SunfishButton OnClick="@(() => { _dialogVisible = false; })">Close</SunfishButton>
    </DialogButtons>
</SunfishDialog>

@code {
    SunfishDialog DialogRef;
...
* [Live Dialog Demos](https://demos.sunfish.dev/blazor-ui/dialog/overview)
* [Dialog API Reference](slug:Sunfish.Blazor.Components.SunfishDialog)
