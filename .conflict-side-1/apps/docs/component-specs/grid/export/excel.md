...
tags: sunfish,blazor,grid,export,excel
published: True
position: 5
components: ["grid"]
---
...
| --- | --- | --- |
| `SaveAsExcelFileAsync` | `ValueTask` | Sends the exported Excel file to the browser for download. You can pass [`GridExcelExportOptions`](slug:Sunfish.Components.DataGrid.GridExcelExportOptions) to customize the export. |
| `ExportToExcelAsync` | `Task<MemoryStream>` | Returns the exported data as a `MemoryStream`. The stream itself is finalized, so that the resource does not leak. To read and work with the stream, clone its available binary data to a new `MemoryStream` instance. You can pass [`GridExcelExportOptions`](slug:Sunfish.Components.DataGrid.GridExcelExportOptions) to customize the export. |
...
<SunfishButton OnClick="@(async () => await GridRef.SaveAsExcelFileAsync())">Download the excel file</SunfishButton>
<SunfishButton OnClick="@GetTheDataAsAStream">Get the Exported Data as a MemoryStream</SunfishButton>
<SunfishButton OnClick="@(async () => await SaveAsExcelWithOptions())">Download Excel with Options</SunfishButton>
<SunfishButton OnClick="@(async () => await ExportToExcelWithOptions())">Get Excel Data with Options</SunfishButton>
...
<SunfishDataGrid @ref="@GridRef"
    ...
    <GridExport>
        <GridExcelExport FileName="sunfish-grid-export" AllPages="@ExportAllPages" />
    </GridExport>
