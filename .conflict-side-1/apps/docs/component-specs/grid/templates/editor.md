...
tags: sunfish,blazor,grid,templates,editor
published: True
position: 15
components: ["grid"]
---
...
````RAZOR
<EditorTemplate>
    <SunfishTextBox Value="@myModel.MyField"
...
````RAZOR.skip-repl
<EditorTemplate>
    <SunfishTextArea @bind-Value="@((Product)context).Description" />
</EditorTemplate>
````
...
<EditorTemplate>
    @{
        ...
        <SunfishTextArea @bind-Value="@editProduct.Description" />
    }
</EditorTemplate>
````
...
````RAZOR
<SunfishDataGrid Data="@Products"
             EditMode="@GridEditMode.Inline"
             OnUpdate="@OnGridUpdate">
        <SunfishGridColumn Field="@nameof(Product.Name)" Width="200px" />
        <SunfishGridColumn Field="@nameof(Product.Description)" Width="200px">
            <EditorTemplate>
                @{
                    var item = (Product)context;

                    <div @onkeydown:stopPropagation>
                        <SunfishTextArea @bind-Value="@item.Description"
                                         DebounceDelay="0" />
                    </div>
                }
            </EditorTemplate>
        </SunfishGridColumn>
        <SunfishGridColumn Field="@nameof(Product.HTMLDescription)">
            <Template>
                @{
                    var item = (Product)context;
            ...
            <EditorTemplate>
                @{
                    ...
                    <div @onkeydown:stopPropagation>
                        <SunfishEditor @bind-Value="@item.HTMLDescription"></SunfishEditor>
                    </div>
                }
            </EditorTemplate>
        </SunfishGridColumn>
...
<SunfishDataGrid Data=@MyData EditMode="@GridEditMode.Inline" Pageable="true" Height="300px" OnUpdate="@UpdateHandler">
        <SunfishGridColumn Field=@nameof(SampleData.ID) Editable="false" Title="ID" />
        <SunfishGridColumn Field=@nameof(SampleData.Name) Title="Name" />
        <SunfishGridColumn Field=@nameof(SampleData.Role) Title="Position">
            <EditorTemplate>
                @{
...
<SunfishDataGrid Data=@MyData EditMode="@GridEditMode.Inline" Pageable="true" Height="500px" OnUpdate="@UpdateHandler">
        <SunfishGridColumn Field=@nameof(Employee.ID) Editable="false" Title="ID" />
        <SunfishGridColumn Field=@nameof(Employee.Name) Title="Name" />
        <SunfishGridColumn Field=@nameof(Employee.RoleId) Title="Position">
            <EditorTemplate>
                @{
                    CurrentlyEditedEmployee = context as Employee;
                    <SunfishDropDownList Data="@Roles"
