...
tags: sunfish, blazor, grid, editing
published: True
...
The Sunfish Grid for Blazor supports create, update, and delete operations (CRUD) with different modes and user experience. The component also supports built-in `DataAnnotations` or custom validation. This page describes:
...
1. Set the `EditMode` parameter to a [member of the `GridEditMode` enum](slug:sunfish.blazor.grideditmode). The default `EditMode` parameter value is `None` and the built-in [`Add` and `Edit` commands](slug://components/grid/columns/command#built-in-commands) don't work.
...
````RAZOR.skip-repl
<SunfishDataGrid EditMode="@GridEditMode.Popup"
...
* By using the [Grid keyboard navigation](https://demos.sunfish.dev/blazor-ui/grid/keyboard-navigation).
...
All events in the [Events table](#events), except `OnModelInit`, provide a [`GridEditEventArgs<TItem>`](slug:Sunfish.Components.DataGrid.GridEditEventArgs-1) event argument. It exposes the following properties:
...
````RAZOR.skip-repl
<SunfishGridColumn Field="@nameof(Race.StartDateTime)"
...
You can control whether a newly added item appears at the top or bottom of the Grid. Use the [`NewRowPosition`](https://sunfish.dev/blazor-ui/documentation/api/sunfish.blazor.gridnewrowposition) parameter to specify the position. This parameter does not affect Popup edit mode, which always displays a dialog for new items.
...
````RAZOR.skip-repl
<SunfishDataGrid @ref="@GridRef" Data="@GridData" EditMode="@GridEditMode.Inline" ...>
    ...
</SunfishDataGrid>

@code {
    private SunfishDataGrid<Employee> GridRef { get; set; } = default!;
...
> The `ConfirmDelete` parameter uses the browser's native `window.confirm` dialog, not a Sunfish `SunfishDialog` component. To use a custom dialog, handle the `OnDelete` event and manage confirmation in application code.
...
* [Online Grid Demos](https://demos.sunfish.dev/blazor-ui/grid/editing-inline)
