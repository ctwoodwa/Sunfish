...
description: Highlight rows and cells in the Sunfish Blazor Grid to draw attention to important data.
tags: sunfish,blazor,grid,highlight,highlighting
published: true
position: 40
components: ["grid"]
---
...
The Sunfish [Blazor Grid](slug:grid-overview) enables you to highlight rows and cells programmatically. Use highlighting to draw attention to important data, indicate status, or visually group related records. Highlighting does not require user interaction and is fully controlled by the application logic.
...
See [Grid Highlighting API Reference](slug:sunfish.blazor.components.HighlightedCellDescriptor) for details about these parameters and the `GridHighlightedCellDescriptor` type.
...
<SunfishDataGrid Data="@GridData"
             ...
             Pageable="true">
        <SunfishGridColumn Field="@nameof(Product.ProductId)" />
        <SunfishGridColumn Field="@nameof(Product.ProductName)" />
        <SunfishGridColumn Field="@nameof(Product.UnitPrice)" />
        <SunfishGridColumn Field="@nameof(Product.UnitsInStock)" />
        <SunfishGridColumn Field="@nameof(Product.CreatedAt)" />
...
* [Highlighting API Reference](slug:sunfish.blazor.components.HighlightedCellDescriptor)