...
tags: sunfish,blazor,grid,observable,data,new,collection
published: True
position: 53
components: ["grid"]
---
...
One of the key features of Sunfish UI for Blazor Grid component is its ability to refresh the displayed data. This ensures that the Grid always presents the most up-to-date information, no matter if you are working with large datasets or frequently changing data sources. Understanding how to efficiently refresh the data in the Grid is crucial for maintaining both optimal performance and user experience. This guide covers various methods and best practices for refreshing data in the Sunfish UI for Blazor Grid, helping you choose the right approach for your specific application needs related to [Blazor DataGrid](https://demos.sunfish.dev/blazor-ui/grid/overview) live update.
...
You can refresh the Grid data by using the `Rebind` method exposed to the reference of the SunfishDataGrid. If you have manually defined the [OnRead event](slug:components/grid/manual-operations) the business logic defined in its event handler will be executed. 

````RAZOR
@* Clicking on the Rebind button will change the Name of the first item in the Grid and refresh the data *@

<SunfishButton OnClick="@RebindGrid">Rebind the Grid</SunfishButton>
...
@code {
    private SunfishDataGrid<Employee> GridRef { get; set; }
...
<SunfishButton OnClick="@AddEmployee">Add employee</SunfishButton>

<SunfishButton OnClick="@RemoveEmployee">Remove last employee</SunfishButton>

<SunfishDataGrid Data="@MyData" Height="400px"
             Pageable="true" Sortable="true">
        <SunfishGridColumn Field="@(nameof(SampleData.Id))" Width="120px" />
        <SunfishGridColumn Field="@(nameof(SampleData.Name))" Title="Employee Name" />
        <SunfishGridColumn Field="@(nameof(SampleData.Team))" Title="Team" />
        <SunfishGridColumn Field="@(nameof(SampleData.HireDate))" Title="Hire Date" />
</SunfishDataGrid>
...
<SunfishButton OnClick="@LoadNewData">Load new data</SunfishButton>
...
<SunfishButton OnClick="@RefreshThroughState">Call OnRead to refresh Grid</SunfishButton>
...
@code {
    ...
    SunfishDataGrid<Employee> GridRef { get; set; }
    ...
    protected async Task ReadItems(GridReadEventArgs args)
    {
        ...
        // see more at https://docs.sunfish.dev/blazor-ui/components/grid/manual-operations
...
## See Also
  ...
  * [Live Demos](https://demos.sunfish.dev/blazor-ui)
