...
tags: sunfish,blazor,calendar,selection,select date,multiple selection
published: True
position: 2
components: ["calendar"]
---
...
To control how many dates the user can select, use the `SelectionMode` property. It takes a member of the `Sunfish.Blazor.CalendarSelectionMode` enum and can be:
* `Single`
* `Multiple`
* `Range`
...
````RAZOR
<SunfishCalendar @bind-Date="@startDate"
...
````RAZOR
<SunfishCalendar @ref="@CalendarRef"
...
@code {
    ...
    private SunfishCalendar CalendarRef { get; set; }
...
<SunfishCalendar SelectionMode="@CalendarSelectionMode.Multiple"
...
@code {
    ...
    private Sunfish.Blazor.Components.SunfishCalendar multipleSelCalendar;
...
## See Also
  ...
  * [Live Demo: Calendar Selection](https://demos.sunfish.dev/blazor-ui/calendar/selection)
