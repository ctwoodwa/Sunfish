...
public class SunfishSyncStatusIndicatorA11yTests : IClassFixture<SunfishSyncStatusIndicatorA11yTests.Ctx>
{
    private readonly Ctx _ctx;

    public SunfishSyncStatusIndicatorA11yTests(Ctx ctx) => _ctx = ctx;

    [Fact]
    public async Task SunfishSyncStatusIndicator_HealthyDefault_HasNoModeratePlusAxeViolations()
    {
        var rendered = _ctx.Bunit.Render<SunfishSyncStatusIndicator>(p => p
            .Add(c => c.Label, "Synced"));
        ...
        try
        {
            ...
            Assert.True(moderatePlus.Count == 0,
                $"SunfishSyncStatusIndicator surfaced {moderatePlus.Count} moderate+ axe violation(s): " +
