...
tags: sunfish,blazor,grid,filtering,filter,menu
published: True
position: 10
components: ["grid"]
---
...
Set the `FilterMode` parameter of the Sunfish Grid to `GridFilterMode.FilterMenu`.

>caption Filter Menu in Sunfish Grid

````RAZOR
@* Filter menu in the column header *@

<SunfishDataGrid Data=@GridData FilterMode="@GridFilterMode.FilterMenu"
			 Pageable="true" Height="400px">
		<SunfishGridColumn Field=@nameof(Employee.Name) />
		<SunfishGridColumn Field=@nameof(Employee.AgeInYears) Title="Age" />
...
<SunfishDataGrid Data="@MyData"
        ...
        <SunfishGridColumn DefaultFilterOperator="FilterOperator.IsEqualTo"
                    Field="@(nameof(SampleData.Id))" Width="120px" />
        <SunfishGridColumn DefaultFilterOperator="FilterOperator.StartsWith"
                    Field="@(nameof(SampleData.Name))" Title="Employee Name" />
        <SunfishGridColumn DefaultFilterOperator="FilterOperator.Contains"
                    Field="@(nameof(SampleData.Team))" Title="Team" />
        <SunfishGridColumn DefaultFilterOperator="FilterOperator.IsGreaterThanOrEqualTo"
...
You can switch between [CheckBoxList](slug:grid-checklist-filter) and a `Menu` filtering layout for a particular `<SunfishGridColumn>` by setting the `FilterMenuType` to `FilterMenuType.Menu` or `FilterMenuType.CheckBoxList`.
...
## See Also
  ...
  * [Live Demo: Grid Filter Menu](https://demos.sunfish.dev/blazor-ui/grid/filter-menu)
