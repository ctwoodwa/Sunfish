...
tags: sunfish,blazor,grid,selection,overview
published: True
position: 0
components: ["grid"]
---
...
You can configure the selection behavior by setting the Grid `SelectionMode` parameter to a member of the `Sunfish.Components.DataGrid.GridSelectionMode` enum. The Grid supports the following selection modes:
