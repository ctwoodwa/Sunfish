...
tags: sunfish,blazor,grid,selection,rows
...
Check the [Grid Keyboard navigation demo](https://demos.sunfish.dev/blazor-ui/grid/keyboard-navigation) for detailed information about selecting rows with the keyboard.
...
1. Optionally, add a [checkbox column](slug:components/grid/columns/checkbox) directly inside `<SunfishDataGrid>`. The `GridCheckboxColumn` provides [additional configuration settings related to selection](slug:components/grid/columns/checkbox#parameters).
...
* [Live Demo: Grid Row Selection](https://demos.sunfish.dev/blazor-ui/grid/row-selection)
