...
tags: sunfish,blazor,chart,label,template,format
published: true
position: 22
components: ["charts"]
---
...
<SunfishChart>
    <ChartSeriesItems>
        <SunfishChartSeries Type="ChartSeriesType.Line"
                     Data="@ChartData"
                     Name="Revenue"
...
> This section changed for product version 4.5.0. If you are using an older version, then [download the PDF documentation](https://sunfish.dev/account/downloads/product-download?product=BLAZOR) for your version, or [browse an older version of this documentation page](https://github.com/ctwoodwa/sunfish-archive/blazor-docs/blob/4.4.0/components/chart/labels-template-and-format.md#templates).

To set a template for Chart labels, use the `Template` parameter in the corresponding inner `...Labels` tag. For example, set `Template` to `<ChartSeriesLabels>` inside `<SunfishChartSeries>`, or to `<ChartValueAxisLabels>` inside `<ChartValueAxis>`.
...
<SunfishChart>
    ...
    <ChartSeriesItems>
        <SunfishChartSeries Type="ChartSeriesType.Scatter"
                     Data="@Series1Data"
...
* [Complex logic in Chart label format](https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/chart/label-template)
