...
tags: sunfish,blazor,mediaquery,integration,chart,grid,form
published: True
position: 1
...
You can integrate the SunfishMediaQuery component with our existing components. This article provides examples on the most common scenarios:
...
You can hide or more columns in the Grid based on the dimensions of the browser window by using the SunfishMediaQuery component and the [Visible parameter](slug:components/grid/columns/bound#grid-bound-column-parameters) of the Grid column.

>tip You can use similar approach for the Sunfish TreeList in order to hide some of the component columns on small devices. You can even replace the entire components with other components that have a simpler layout and limited functionality, such as a ListView, for small devices.
...
<SunfishMediaQuery Media="(max-width: 1023px)" OnChange="@( (doesMatch) => IsMediumDown = doesMatch )"></SunfishMediaQuery>

<SunfishGrid Data="@MyData"
...
>note You can also see the <a href="https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/chart/responsive-chart" target="_blank">Responsive Chart demo application</a> for additional examples.

````RAZOR
@* Resize the chart based on the browser size so it matches the corresponding responsive layout *@

<SunfishMediaQuery Media="(max-width: 767px)" OnChange="@OnChangeHandler"></SunfishMediaQuery>

<div style="width: @Width; height: @Height">

    <SunfishChart Width="100%" Height="100%" @ref="@ChartRef">
        ...
        <ChartLegend Position="@Sunfish.Blazor.ChartLegendPosition.Right">
        </ChartLegend>
...
@code {
    ...
    public SunfishChart ChartRef { get; set; }
...
<SunfishMediaQuery Media="@SmallScreenMediaQuery" OnChange="@( (bool matches) => { IsSmallScreen = matches; ConfigureForm(); } )" />
<SunfishMediaQuery Media="@MediumScreenMediaQuery" OnChange="@( (bool matches) => { IsMediumScreen = matches; ConfigureForm(); } )" />
<SunfishMediaQuery Media="@LargeScreenMediaQuery" OnChange="@( (bool matches) => { IsLargeScreen = matches; ConfigureForm(); } )" />
<SunfishMediaQuery Media="@ExtraLargeScreenMediaQuery" OnChange="@( (bool matches) => { IsExtraLargeScreen = matches; ConfigureForm(); } )" />
...
* [Live Demo: MediaQuery and Grid Integration](https://demos.sunfish.dev/blazor-ui/mediaquery/grid-integration)
