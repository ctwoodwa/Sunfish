...
tags: sunfish,blazor,TreeList,filtering,filter,row
published: True
position: 5
components: ["treelist"]
---
...
To enable the filter row set the `FilterMode` property of the treelist to `Sunfish.Blazor.TreeListFilterMode.FilterRow`.
...
>caption Filter Row in Sunfish TreeList

````RAZOR
@* Filter row mode *@

<SunfishTreeList Data="@Data" FilterMode="@TreeListFilterMode.FilterRow"
