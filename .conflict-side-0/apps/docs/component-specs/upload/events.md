...
slug: upload-events
tags: sunfish,blazor,upload,async,events
published: true
position: 20
components: ["upload"]
---
...
This article describes the events of the Sunfish Upload for Blazor.
...
| `Operation` | [`UploadOperationType`](slug:Sunfish.Blazor.UploadOperationType) enum | Can be `Upload` or `Remove`. |
...
| `Status` | [`UploadFileStatus` enum](slug:Sunfish.Blazor.UploadFileStatus) | The current status of the file in the context of the Upload component (`Selected`, `Uploading`, `Uploaded`, `Failed`). |
...
````RAZOR
<SunfishUpload OnCancel="@OnUploadCancel" />
...
````RAZOR
<SunfishUpload OnClear="@OnUploadClear" />
...
````RAZOR.skip-repl
<SunfishUpload OnError="@OnUploadError" />
...
````RAZOR
<SunfishUpload OnProgress="@OnUploadProgress" />
...
````RAZOR
<SunfishUpload OnRemove="@OnUploadRemove" />
...
````RAZOR
<SunfishUpload OnSelect="@OnUploadSelect" />
...
@code {
    private async Task OnUploadSelect(UploadSelectEventArgs args)
    {
        foreach (var file in args.Files)
        {
            ...
            // Change the file name that is displayed in the SunfishUpload component.
...
````RAZOR
<SunfishUpload OnSuccess="@OnUploadSuccess" />
...
<SunfishUpload OnUpload="@OnUploadHandler" />
...
* [Live Demo: Upload Events](https://demos.sunfish.dev/blazor-ui/upload/events)
