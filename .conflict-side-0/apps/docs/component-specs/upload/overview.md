...
tags: sunfish,blazor,upload,async,overview
published: True
position: 0
components: ["upload"]
---
...
````CS
<SunfishUpload SaveUrl="/api/upload/save"
...
The `UploadController` class below assumes that the project name and namespace is `SunfishBlazorApp`.
...
namespace SunfishBlazorApp.Controllers
{
...
The Sunfish Upload component makes XHR requests from the browser to the designated endpoints. If needed, use the [`OnUpload` and `OnRemove` events](slug:upload-events) to add headers, authentication tokens and custom data to the request.
...
You can use the functionality of the built-in template and modify the appearance of the **Select files...** button. [Read more about the Sunfish Upload templates...](slug:upload-templates)
...
Additionally, you can define an external drop zone by using the [Sunfish UI for Blazor DropZone component](slug:dropzone-overview).

## Upload Parameters

The following table lists the Upload parameters. Also check the [Upload API Reference](slug:Sunfish.Blazor.Components.SunfishUpload) for a full list of properties, methods and events.
...
````RAZOR
<p>
    <SunfishButton OnClick="@SelectFiles">Open File Selection Dialog</SunfishButton>
    <SunfishButton OnClick="@Clear">Clear File List</SunfishButton>
    <SunfishButton OnClick="@Upload">Start Upload</SunfishButton>
</p>

<SunfishUpload @ref="@UploadRef"
               SaveUrl="/api/upload/save"
               RemoveUrl="/api/upload/remove"
               AutoUpload="false" />

@code {
    private SunfishUpload UploadRef { get; set; }
...
* [Live Upload Demos](https://demos.sunfish.dev/blazor-ui/upload/overview)
* [Upload API](slug:Sunfish.Blazor.Components.SunfishUpload)
