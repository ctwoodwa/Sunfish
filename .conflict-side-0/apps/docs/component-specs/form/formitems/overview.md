...
slug: form-formitems
tags: sunfish,blazor,form,edit,formitems
published: True
position: 0
components: ["form"]
---
...
Each custom editor is expressed through the `FormItem` tag. You can define a collection of those editors in the `FormItems` tag that is a child of the `SunfishForm` tag. You can find some [code examples](#example) below.
...
* Use a Sunfish input component inside a [Form item `<Template>`](slug:form-formitems-template). Define two-way binding for the `Value` parameter, or use `Value`, `ValueChanged`, and `ValueExpression`.
        ...
        <FormItem Field="@nameof(Person.Name)">
            <Template>
                <label class="k-label k-form-label" style="color: var(--kendo-color-success)">
                    Sunfish component - re-render inside and outside the Form.
                </label>
                <div class="k-form-field-wrap">
                    <SunfishTextBox @bind-Value="@Employee.Name" DebounceDelay="0" />
                    <SunfishValidationMessage For="@( () => Employee.Name )" />
                </div>
            </Template>
        </FormItem>
        <FormItem Field="@nameof(Person.Name)">
...
@code {
    ...
    // See https://sunfish.dev/blazor-ui/documentation/knowledge-base/inputs-validation-child-component
    [Parameter]
...
* [Online demo: Form Items](https://demos.sunfish.dev/blazor-ui/form/overview)
