...
tags: sunfish,blazor,popover,pop,over
published: True
position: 0
components: ["popover"]
---
...
>caption Basic Sunfish Popover for Blazor

````RAZOR
<SunfishPopover @ref="@PopoverRef"
                AnchorSelector=".popover-target"
                ShowOn="@PopoverShowOn.Click"
                Position="@PopoverPosition.Bottom"
                Offset="20">
    <PopoverContent>
        Sunfish Popover for Blazor
    </PopoverContent>
    <PopoverActions>
        <SunfishButton OnClick="@( () => PopoverRef?.Hide() )"
                       Icon="@SvgIcon.X">Close</SunfishButton>
    </PopoverActions>
</SunfishPopover>

<SunfishButton Class="popover-target">Show Popover Automatically</SunfishButton>

<SunfishButton OnClick="@( () => PopoverRef?.Show() )">Show Popover Programmatically</SunfishButton>

@code{
    private SunfishPopover? PopoverRef { get; set; }
}
````
...
The Blazor Popover provides parameters to configure the component. Also check the [Popover API Reference](slug:Sunfish.Blazor.Components.SunfishPopover) for a full list of properties.
...
| Method  | Description |
|---------|-------------|
| `Refresh` | Use this method to programmatically re-render the Popover. <br /> The Popover is rendered as a child of the `SunfishRootComponent`, instead of where it is declared. As a result, it doesn't automatically refresh when its content is updated. In such cases, the `Refresh` method comes in handy to ensure that the Popover content is up-to-date. |
...
<SunfishPopover @ref="@PopoverRef"
    ...
    <PopoverActions>
        <SunfishButton OnClick="@( () => PopoverRef?.Hide() )"
                       Icon="@SvgIcon.X">Hide</SunfishButton>
    </PopoverActions>
</SunfishPopover>

<SunfishButton OnClick="@( () => PopoverRef?.Show() )">Show Popover</SunfishButton>

<SunfishSvgIcon Class="popover-target" Icon="@SvgIcon.QuestionCircle" />
...
* [Live Popover Demos](https://demos.sunfish.dev/blazor-ui/popover/overview)
* [Popover API Reference](slug:Sunfish.Blazor.Components.SunfishPopover)
