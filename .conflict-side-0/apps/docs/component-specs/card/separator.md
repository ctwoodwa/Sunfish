...
slug: card-separator
tags: sunfish,blazor,card,separator
published: True
position: 5
components: ["card"]
---
...
* `Orientation` - `CardOrientation` - defines the orientation of the `CardSeparator`. Takes a member of the `Sunfish.Blazor.CardOrientation` enum (`Horizontal` or `Vertical`).
