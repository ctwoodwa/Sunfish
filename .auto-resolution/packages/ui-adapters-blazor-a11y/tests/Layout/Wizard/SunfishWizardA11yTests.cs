...
public class SunfishWizardA11yTests : IClassFixture<SunfishWizardA11yTests.Ctx>
{
    private readonly Ctx _ctx;

    public SunfishWizardA11yTests(Ctx ctx) => _ctx = ctx;

    [Fact]
    public async Task SunfishWizard_DefaultRender_HasNoModeratePlusAxeViolations()
    {
        var rendered = _ctx.Bunit.Render<SunfishWizard>();
        ...
        try
        {
            ...
            Assert.True(moderatePlus.Count == 0,
                $"SunfishWizard surfaced {moderatePlus.Count} moderate+ axe violation(s): " +
