...
tags: sunfish,blazor,tile,layout,dashboard,resize
published: True
position: 15
...
1. Set the `Resizable` parameter of the main `SunfishTileLayout` tag to `true`.

2. Set the  `RowHeight` and `ColumnWidth` parameters of the `SunfishTileLayout`. The provided values must be in absolute units—this allows for the layout to correctly calculate the position of each tile during resizing.
...
````RAZOR
<SunfishTileLayout ColumnWidth="200px"
                   RowHeight="150px"
                   Width="700px"
                   Columns="3"
                   Resizable="true">
