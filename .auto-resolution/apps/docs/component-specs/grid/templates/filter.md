...
tags: sunfish,blazor,grid,templates,filter
published: True
position: 35
components: ["grid"]
---
...
You can find more examples in the [Live Demo: Custom Filter Row](https://demos.sunfish.dev/blazor-ui/grid/custom-filter-row) that is available in your local installation under the `demos` folder. Also check [How to implement a filter operator dropdown list in a Filter Row Template](slug:grid-kb-filter-operator-dropdown) in order to mimic the default filter row UI.
...
<SunfishDataGrid Data=@GridData FilterMode="@GridFilterMode.FilterRow"
             Height="400px" Width="700px" Pageable="true">
        <SunfishGridColumn Field="Id" Filterable="false" Width="80px" />

        <SunfishGridColumn Field="Price" Width="400px">
            <FilterCellTemplate>
                @{
                ...
                <SunfishNumericTextBox Id="min"
                                       @bind-Value="@MinValue"
                                       OnChange="@SetupFilterRule">
                </SunfishNumericTextBox>
                <label for="min">Max:&nbsp;</label>
                <SunfishNumericTextBox Id="max"
                                       @bind-Value="@MaxValue"
                                       OnChange="@SetupFilterRule">
                </SunfishNumericTextBox>
                <SunfishButton ButtonType="ButtonType.Button"
        ...
        <SunfishGridColumn Field="ProductName" Title="Product" Filterable="false" />
</SunfishDataGrid>
...
To customize the filter menu, use the `<FilterMenuTemplate>` tag of the `<SunfishGridColumn>`. The `Filter` and `Clear` buttons are still available below the template.
...
You can find more examples in the [Live Demo: Custom Filter Menu](https://demos.sunfish.dev/blazor-ui/grid/custom-filter-menu) that is available in your local installation under the `demos` folder.
...
<SunfishDataGrid Data="@GridData"
             ...
             Pageable="true"
             Width="600px">
        <SunfishGridColumn Field="@(nameof(Product.Name))" Title="Product" Filterable="false" />
        <SunfishGridColumn Field="@(nameof(Product.Size))">
            <FilterMenuTemplate>
                @foreach (var size in Sizes)
                {
                    <div>
                        <SunfishCheckBox Value="@(IsCheckboxInCurrentFilter(context.FilterDescriptor, size))"
                                         TValue="bool"
...
<SunfishDataGrid Data="@GridData"
             ...
             Sortable="true"
             FilterMode="@GridFilterMode.FilterMenu">
        <SunfishGridColumn Field="Name" Title="Product Name" />
        <SunfishGridColumn Field="Price">
            <FilterMenuButtonsTemplate Context="filterContext">
                <SunfishButton OnClick="@(async _ => await filterContext.FilterAsync())">Filter </SunfishButton>
                <SunfishButton OnClick="@(() => SetPredefinedFilterAsync(filterContext))">Select >= 55</SunfishButton>
                <SunfishButton OnClick="@(() => ClearFilterAsync(filterContext))">Clear</SunfishButton>
            </FilterMenuButtonsTemplate>
        </SunfishGridColumn>
        <SunfishGridColumn Field="@nameof(Product.Released)" />
        <SunfishGridColumn Field="@nameof(Product.Discontinued)" />
</SunfishDataGrid>
...
* [Live Demo: Grid Custom Filter Row](https://demos.sunfish.dev/blazor-ui/grid/custom-filter-row)
* [Live Demo: Grid Custom Filter Menu](https://demos.sunfish.dev/blazor-ui/grid/custom-filter-menu)
