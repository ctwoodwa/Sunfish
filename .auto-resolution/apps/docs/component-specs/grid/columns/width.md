...
tags: sunfish,blazor,grid,column,width
published: True
position: 30
components: ["grid"]
---
...
You can set the Grid column `Width` parameter in any CSS unit, such as `px`, `%`, `vw`, `em`, `rem`. Unit-less `Width` values are not supported. You can read more on how to set sizes in Sunfish components in the [Dimensions](slug:common-features/dimensions) article.
        ...
        <SplitterPane>
            <SunfishDataGrid Data="@GridData"
                ...
                <GridAggregates>
                    ...
                    <SunfishGridColumn Field="@nameof(Product.Id)" Width="90px" />
                    <SunfishGridColumn Field="@nameof(Product.Name)">
                        <FooterTemplate>
                            Count: @context.Count
                        </FooterTemplate>
                    </SunfishGridColumn>
                    <SunfishGridColumn Field="@nameof(Product.Price)" Width="110px">
                        <FooterTemplate>
                            Average: @context.Average
                        </FooterTemplate>
                    </SunfishGridColumn>
                    <SunfishGridColumn Field="@nameof(Product.Quantity)" Width="130px">
                        <FooterTemplate>
                            Sum: @context.Sum
                        </FooterTemplate>
                    </SunfishGridColumn>
                    <SunfishGridColumn Field="@nameof(Product.ReleaseDate)" DisplayFormat="{0:D}" />
                    <SunfishGridColumn Field="@nameof(Product.IsActive)" Width="120px" />
            </SunfishDataGrid>
        </SplitterPane>
    </SplitterPanes>
</SunfishSplitter>
