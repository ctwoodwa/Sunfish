...
slug: chart-events
tags: sunfish,blazor,chart,events,event
published: true
position: 33
components: ["charts"]
---
...
This article describes the available events for the Sunfish Chart for Blazor:
...
<SunfishChart OnAxisLabelClick="@OnChartAxisLabelClick">
    <ChartSeriesItems>
...
<SunfishRadioGroup Data="@ClickModes" @bind-Value="@SelectedMode" />

<SunfishChart OnLegendItemClick="@OnLegendClickHandler"
              Transitions="false">
    <ChartSeriesItems>
...
The `OnSeriesClick` event fires as a response to the user click on a `<SunfishChartSeries>`.
...
* `SeriesIndex` - provides the index of the `<SunfishChartSeries>` the data point belongs to.

* `SeriesName` - bound to the Name parameter of the `<SunfishChartSeries>` the data point belongs to.
...
<SunfishChart OnSeriesClick="@OnSeriesClickHandler">
...
{
    <div class="text-center">
        <SunfishGrid Data="@GridData" AutoGenerateColumns="true"
...
* [Live Demo: Chart Events](https://demos.sunfish.dev/blazor-ui/chart/events)
