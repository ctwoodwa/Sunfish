...
tags: sunfish,blazor,gantt,chart,tree,filtering,filter,menu,row
published: True
position: 0
components: ["gantt"]
---
...
* [`GanttFilterMode.FilterRow`](slug:gantt-filter-row) - a row of filter options is rendered below the column headers

* [`GanttFilterMode.FilterMenu`](slug:gantt-filter-menu) - the column headers render a funnel icon in each filterable column header. Clicking the icon opens a popup with a text input, Filter button, and Clear button. Set `FilterMode="GanttFilterMode.FilterMenu"` to enable.
