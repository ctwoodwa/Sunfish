...
tags: sunfish,blazor,gantt,data,bind,databind,databinding,basics
published: True
position: 0
components: ["gantt"]
---
...
The Blazor Gantt Tree provides various parameters to configure its items. Also, check the [Gantt public API](slug:Sunfish.Components.DataDisplay.SunfishGantt-1).
...
The properties of a Gantt Tree item match directly to a field of the model the treelist is bound to. Provide that relationship by providing the name of the field from which the corresponding information is to be taken. To do this, in the main `SunfishGantt` tag, use the parameters described below:
...
All field mapping parameters are customizable. If your model uses different property names, set these parameters on the `SunfishGantt` component to match.
