...
description: Learn about the animation options for the Sunfish Window component in Blazor.
slug: window-animations
tags: sunfish,blazor,window,animations
published: True
position: 4
components: ["window"]
---
...
The Sunfish Window component for Blazor provides an option to control the opening animations to enhance the user experience. You can configure the animation type and duration using the following parameters:
...
<div class="k-d-flex k-align-items-stretch k-gap-5">
    ...
    <div class="k-d-flex k-flex-col">
        ...
        <SunfishNumericTextBox @bind-Value="@Duration"
                               Width="130px"
                               Id="duration" />
    </div>

    <div class="k-align-self-flex-end">
        <SunfishButton OnClick="@(() => Visible = !Visible)">@(Visible ? "Hide Window" : "Show Window")</SunfishButton>
    </div>
</div>

<SunfishWindow @bind-Visible="@Visible"
               Height="300px"
               Width="300px"
               @bind-Top="@Top"
               @bind-Left="@Left"
