...
tags: sunfish,blazor,form,layout
published: True
position: 6
components: ["form"]
---
...
>tip The content on this page applies to Sunfish UI for Blazor version **4.2.0** and above.
...
Sunfish UI for Blazor version 4.2.0 introduced one new Form parameter and two new components that allow mixing Form groups and items with other content, such as HTML markup or Razor components:

@[template](/_contentTemplates/common/parameters-table-styles.md#table-layout)

| Feature | Description |
| --- | --- |
| [`FormItemsTemplate`](#formitemstemplate) | A `RenderFragment` parameter of the `SunfishForm` component, so it's usually used as a child tag. |
| [`<SunfishFormGroupRenderer>`](#form-group-renderer) | A component that renders a specific *Form group*. It can occur anywhere inside `<FormItemsTemplate>`. |
| [`<SunfishFormItemRenderer>`](#form-item-renderer) | A component that renders a specific *Form item*. It can occur inside `<FormItemsTemplate>` or the `Template` of a `<SunfishFormGroupRenderer>`. |
...
* [`<SunfishFormGroupRenderer>`](#form-group-renderer) defines the **rendering place** of a specific form **group**.
* [`<SunfishFormItemRenderer>`](#form-item-renderer) defines the **rendering place** of a specific form **item**.
...
Be aware of [nested form contexts](#nested-form-contexts) when using `SunfishFormGroupRenderer`.


## Form Group Renderer

The `SunfishFormGroupRenderer` component defines the rendering place of a Form group inside the `FormItemsTemplate`. The component has the following parameters:
...
The `SunfishFormItemRenderer` component defines the rendering place of a Form item inside the `FormItemsTemplate`. The component has the following parameters:
...
>tip `SunfishFormItemRenderer` is unrelated to the [template of a single `<FormItem>`](slug:form-formitems-template). The two features are used separately and independently.
...
1. Inside `<FormItemsTemplate>`, define a `<SunfishFormGroupRenderer>` and `<SunfishFormItemRenderer>` for each group and item that exists in `<FormItems>`. Place each Renderer tag at the desired place. This steps also applies to `<FormAutoGeneratedItems />`, which need Renderers too.
1. If using conditional statements to render `<FormItem>` tags, then define these tags unconditionally. Instead, use conditional logic for `<SunfishFormItemRenderer>` tags inside `<FormItemsTemplate>`.
1. If [using `<FormItem>` instances inside a nested Razor component](slug:form-kb-use-items-in-custom-component), then see the linked KB article and:
    ...
    * Use `<SunfishFormItemRenderer>` inside the nested component. Consume the new `IFormItem` parameter in the `Item` parameter of the Renderer.
...
````RAZOR.skip-repl
<SunfishFormItemRenderer Item="@NestedFormItem" />
...
* [Form Templates online demo](https://demos.sunfish.dev/blazor-ui/form/templates).
    ...
    <FormItemsTemplate Context="formContext">
        ...
        @foreach (IFormItemBase item in formContext.Items)
        {
            if (item is IFormGroup) // only if using FormGroups
            {
                ...
                <SunfishFormGroupRenderer Group="@groupItem">
                    <Template Context="groupContext">
                        <div class="form-group-wrapper">
                            ...
                            @foreach (IFormItem singleItem in groupContext.Items)
                            {
                                <div class="form-item-wrapper">
                                    <SunfishFormItemRenderer Item="@singleItem" />
                                </div>
                            }
                        </div>
                    </Template>
            ...
            {
                <div class="form-item-wrapper">
                    <SunfishFormItemRenderer Item="@(item as IFormItem)" />
                </div>
            }
        }
...
<SunfishForm Model="@Employee"
        ...
        <FormItem Field="@nameof(Person.BirthDate)">
            <Template>
                ...
                <div class="k-form-field-wrap">
                    <SunfishDatePicker @bind-Value="@Employee.BirthDate"
                                       Format="d"
                                       Id="birth-date" />
                </div>
            </Template>
    ...
    <FormItemsTemplate Context="formContext">
        ...
        <div class="form-item-wrapper">
            ...
            <SunfishFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Person.Id)) )" />
        </div>
        <div class="form-item-wrapper">
            @* Get Form item by index *@
            <SunfishFormItemRenderer Item="@( formItems[1] )" />
        </div>
        <div class="form-item-wrapper">
            @* Get Form item by Id *@
            <SunfishFormItemRenderer Item="@( formItems.First(x => x.Id == "last-name-item") )" />
        </div>
        <div class="form-item-wrapper">
            <SunfishFormItemRenderer Item="@( formItems.Skip(3).First() )" />
        </div>
        ...
        {
            <div class="form-item-wrapper">
                <SunfishFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Person.DriversLicense)) )" />
            </div>

            <div class="form-item-wrapper">
                <SunfishFormGroupRenderer Group="@( formGroups.First(x => x.Id == "job-info-group") )">
                    <Template Context="groupContext">
                        @{
                            var groupItems = groupContext.Items.OfType<IFormItem>().ToList();
                        }
                        <SunfishFormItemRenderer Item="@( groupItems.First(x => x.Field == nameof(Person.Team)) )" />
                        <SunfishFormItemRenderer Item="@( groupItems.First(x => x.Field == nameof(Person.Salary)) )" />
                    </Template>
                </SunfishFormGroupRenderer>
            </div>
        }
...
<SunfishForm Model="@Employee"
    ...
    <FormItemsTemplate Context="formContext">
        @{
            ...
            {
                <div class="form-item-wrapper">
                    <SunfishFormItemRenderer Item="@formItem" />
                </div>
            }
        }
    </FormItemsTemplate>
...
* [Online Demo: Form Templates](https://demos.sunfish.dev/blazor-ui/form/templates)
