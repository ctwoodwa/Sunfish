...
| Reverse-proxy load test (ADR 0033 gate) | 🟢 | Wave 5.3.C harness shipped — `accelerators/bridge/tests/Sunfish.Bridge.Tests.Integration/Wave53/ReverseProxyLoadBench.cs`, opt-in via `SUNFISH_RUN_BENCH=1`. Exercises 30 sessions × 3 tenants × 100 × 1 KB frames through `TenantWebSocketReverseProxy`. Loopback run 2026-04-23 (see [`_shared/research/reverse-proxy-load-bench-2026-04-23.md`](../../_shared/research/reverse-proxy-load-bench-2026-04-23.md)) clears both [ADR 0033](../../docs/adrs/0033-browser-shell-render-model-and-trust-posture.md) gates: handshake p99 85 ms < 2000 ms, frame-RTT p99 0.5 ms < 500 ms. Real-network validation is a follow-up before first-customer onboarding; loopback is a ceiling, not proof of prod feel. |
