...
Naval-org chart: **CO** (Chris, BDFL) → **XO** (this repo, research) → {**COB** (this repo, sunfish-PM), **dev** (galley primary + Sunfish overflow), **PAO** (book repo, editor) → **Yeoman** (book repo, technical writer)}. Five Claude sessions share filesystem; cannot talk directly.
...
| **dev** (galley primary + Sunfish overflow) | Galley application (TypeScript/Turbo monorepo: web/desktop/api/menubar/mcp + book-server + Python workers + prose tool family). Secondary: pitch in on Sunfish production code when COB asks or CO directs. Opus 4.7 + xhigh — used sparingly. Agent def: `../.claude/agents/dev.md`. |
