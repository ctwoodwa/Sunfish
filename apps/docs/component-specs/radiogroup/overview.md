...
tags: sunfish,blazor,radiobuttongroup,radio,list,overview
published: True
position: 0
...
The <a href="https://sunfish.dev/blazor-ui/radiogroup" target="_blank">Blazor RadioGroup component</a> allows the user to select an option from a predefined set of choices in a list of radio buttons. The radio group is styled according to the Sunfish [Theme](slug:themes-overview). You can also choose the [layout order](slug:radiogroup-layout) and [label position](slug:radiogroup-label-position).

## Creating Blazor RadioGroup

1. Add the `<SunfishRadioGroup>` tag to a Razor file.
...
<SunfishRadioGroup Data="@GenderOptions"
...
@code{
    SunfishRadioGroup<GenderModel, int?> RadioGroupRef { get; set; }
...
The Blazor RadioGroup provides various parameters to configure the component. Also check the [RadioGroup public API](slug:Sunfish.Blazor.Components.SunfishRadioGroup-2).
...
````RAZOR
<SunfishButton OnClick="@FocusLastRadioItem">Focus Last Radio Item</SunfishButton>

<SunfishRadioGroup @ref="@RadioGroupRef"
                   Data="@RadioGroupData"
...
@code{
    private SunfishRadioGroup<ListItem, int?>? RadioGroupRef { get; set; }
...
* [Live RadioGroup Demos](https://demos.sunfish.dev/blazor-ui/radiogroup/overview)
* [RadioGroup API Reference](slug:Sunfish.Blazor.Components.SunfishRadioGroup-2)
