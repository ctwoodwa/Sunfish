...
description: Learn the basics and fundamentals of the Sunfish UI for Blazor PivotGrid.
slug: pivotgrid-overview
tags: sunfish,blazor,pivotgrid
published: True
position: 0
...
The <a href="https://sunfish.dev/blazor-ui/pivotgrid" target="_blank">Blazor PivotGrid component</a>, also known as a pivot table, is a powerful data visualization, statistics and reporting tool that allows you to perform operations and analysis over multi-dimensional pivot data. It can work with local data or a remote XMLA data such as an OLAP cube. The PivotGrid also supports filtering and sorting for the values of the row and column fields.
...
| --- | --- |
| `<SunfishPivotGrid>` | The **PivotGrid** displays the aggregate measures. |
| `<SunfishPivotGridConfigurator>` | The **Configurator** enables end users to add or remove rows, columns and measures. |
| `<SunfishPivotGridConfiguratorButton>` | The **Button** toggles the visibility of the configurator. |
| `<SunfishPivotGridContainer>` | The **Container** wraps the above components. |
...
> * The `<SunfishPivotGridContainer>` is a standard Blazor `RenderFragment` and allows any `ChildContent`. Nevertheless, *do not* place any other custom components or markup inside it.
...
1. (optional) To give users more flexibility, add a Configurator and a toggle Button next to the `<SunfishPivotGrid>` tag:
    * Add a `<SunfishPivotGridConfigurator>`.
    * Add a `<SunfishPivotGridConfiguratorButton>`.
    * Wrap both tags and `<SunfishPivotGrid>` with a `<SunfishPivotGridContainer>`.

> The Sunfish PivotGrid for Blazor [depends on two additional NuGet packages](slug:getting-started/what-you-need#nuget-packages). They are installed automatically when using the [Sunfish NuGet feed](slug:installation/nuget) and must not be used separately with other custom or third-party tools.
...
````RAZOR
<SunfishPivotGridContainer>

    <SunfishPivotGridConfigurator />

    <SunfishPivotGridConfiguratorButton />

    <SunfishPivotGrid Data="@PivotData"
...
The following table lists the `SunfishPivotGrid` parameters.
...
| `ChildContent` | `RenderFragment?` | Child content that accepts `SunfishPivotGridRowField`, `SunfishPivotGridColumnField`, and `SunfishPivotGridMeasureField` components. |
...
> The `RowFields`, `ColumnFields`, and `MeasureFields` list parameters are obsolete. Use child field components (`SunfishPivotGridRowField`, `SunfishPivotGridColumnField`, `SunfishPivotGridMeasureField`) instead.

### Row, Column and Measure Field Parameters

The following table lists parameters of the `SunfishPivotGridRowField`, `SunfishPivotGridColumnField` and `SunfishPivotGridMeasureField` child components.

| Parameter | Type and Default&nbsp;Value | Description |
| --- | --- | --- |
| `AggregateFunction` | `PivotGridAggregateFunction` enum <br /> (`Sum`) | The nature of the calculated aggregate values. Applies to `SunfishPivotGridMeasureField` only. Members: `Sum`, `Count`, `Average`, `Min`, `Max`. |
| `Field` | `string` | The property name on the data item to bind to this field. |
| `Format` | `string?` | The [display format](https://learn.microsoft.com/en-us/dotnet/standard/base-types/standard-numeric-format-strings) of the calculated aggregate values, for example `"C2"` or `"N1"`. Applies to `SunfishPivotGridMeasureField` only. |
| `Title` | `string?` | Display title for the field header. Falls back to `Field` if not set. |


### Configurator Parameters

The following table lists parameters of the `SunfishPivotGridConfigurator` component.
...
The following table lists parameters of the `SunfishPivotGridConfiguratorButton` component.
...
The following table lists parameters of the `SunfishPivotGridContainer` component.

| Parameter | Type | Description |
| --- | --- | --- |
| `Class` | `string` | A custom CSS class for the Container `<div>` element. |
| `ChildContent` | `RenderFragment?` | Primary content, typically a `SunfishPivotGrid`. |
...
````RAZOR
<SunfishPivotGrid @ref="@PivotGridRef" />

<SunfishButton OnClick="@OnButtonClick">Rebind PivotGrid</SunfishButton>

@code {
    private SunfishPivotGrid<PivotModel> PivotGridRef { get; set; } = null!;
...
* [Live PivotGrid Demos](https://demos.sunfish.dev/blazor-ui/pivotgrid/overview)
* [PivotGrid API Reference](slug:Sunfish.Blazor.Components.SunfishPivotGrid-1)
