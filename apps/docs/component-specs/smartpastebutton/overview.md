...
tags: sunfish, blazor, smartpastebutton, ai, form
published: True
position: 0
...
The <a href="https://sunfish.dev/blazor-ui/smartpastebutton" target="_blank">UI for Blazor SmartPasteButton component</a> is an AI-powered utility designed to parse unstructured text data and use the result to populate form fields. It eliminates the "copy-paste-repeat" manual grind by using large language models to map raw text to specific data inputs.

The SmartPasteButton integrates seamlessly with the [Sunfish Form](slug:form-overview), Blazor EditForm, and native HTML forms.

## Creating Blazor SmartPasteButton

### Configuration without IChatClient

1. Use the `<SunfishSmartPasteButton>` tag to add the component to your razor page.
...
@using Sunfish.AI.SmartComponents.Extensions
...
<div style="max-width: 600px; margin-bottom: 16px;">
    ...
    <SunfishTextArea Value="@SampleText" Rows="3" ReadOnly="true" />

    <div style="margin-top: 8px;">
        <SunfishButton OnClick="@CopySampleText" Icon="SvgIcon.Copy">
            Copy
        </SunfishButton>
    </div>
</div>

<SunfishForm Model="@Model" OnSubmit="@HandleSubmit">
    <FormItems>
    ...
    <FormButtons>
        <SunfishSmartPasteButton @ref="@SmartPasteButtonRef"
                                 EnableChatClient="false"
                                 Enabled="@(!Pasting)"
                                 OnRequestStart="@HandleRequestStart">
            Smart Paste
        </SunfishSmartPasteButton>
    </FormButtons>
</SunfishForm>


@code {
    private SunfishSmartPasteButton? SmartPasteButtonRef { get; set; }
...
+1 555 123 4567";
    ...
    private async Task HandleRequestStart(SmartPasteButtonRequestStartEventArgs args)
    {
        ...
        var response = await HttpClient.PostAsJsonAsync(
            "https://demos.sunfish.dev/service/v2/ai/smartpaste/smartpaste",
            payload
        );
...
````
````RAZOR Home.razor
<SunfishSmartPasteButton ChatClientKey="gpt-4.1" />
````

## Form Field Support

The SmartPasteButton works with both native HTML form fields and Sunfish input components. The following Sunfish components are supported:
...
To review all available parameters, methods, and events of the SmartPasteButton component, see the [SmartPasteButton API Reference](https://docs.sunfish.dev/blazor-ui/api/Sunfish.Blazor.Components.SunfishSmartPasteButton#parameters).

## SmartPasteButton Reference and Methods

The SmartPasteButton component exposes several public methods that you can call from your code. For a full list and details, see the [SmartPasteButton API Reference](https://docs.sunfish.dev/blazor-ui/api/Sunfish.Blazor.Components.SunfishSmartPasteButton#methods).
...
* [SmartPasteButton Live Demo](https://demos.sunfish.dev/blazor-ui/smartpastebutton/overview)
* [SmartPasteButton API Reference](/blazor-ui/api/Sunfish.Blazor.Components.SunfishSmartPasteButton)
