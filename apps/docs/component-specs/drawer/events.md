...
slug: drawer-events
tags: sunfish,blazor,drawer,event,events
published: True
position: 25
components: ["drawer"]
---
...
This article explains the events available in the Sunfish Drawer for Blazor:
...
<SunfishDrawer Data="@Data" Expanded="true" MiniMode="true" Mode="DrawerMode.Push"
...
@if (!Expanded)
{
    <SunfishButton OnClick="@(() => DrawerRef.ExpandAsync())" Icon="@SvgIcon.Menu">Expand Drawer</SunfishButton>
}

<SunfishDrawer Expanded="@Expanded"
...
<SunfishButton OnClick="@(() => DrawerRef.ToggleAsync())"
               Icon="@SvgIcon.Menu">Toggle drawer</SunfishButton>
...
@code {
    private SunfishDrawer<DrawerItem> DrawerRef { get; set; }
