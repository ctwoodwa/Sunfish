...
tags: sunfish,blazor,treelist,sorting
published: True
position: 21
components: ["treelist"]
---
...
You can let the user sort by more than one field by setting the `SortMode` parameter to `Sunfish.Blazor.SortMode.Multiple`.

The sorting criteria are stored in a [collection of `SortDescriptor`](slug:common-features-descriptors#sorting).

>caption Enable Sorting in Sunfish TreeList

````RAZOR
Click a column header to sort by its data

<SunfishTreeList Data="@Data" Sortable="true"
...
## See Also
  ...
  * [Live Demo: TreeList Sorting](https://demos.sunfish.dev/blazor-ui/treelist/sorting)
