...
tags: sunfish,blazor,wizard,buttons
published: True
position: 5
components: ["wizard"]
---
...
This article provides details on the buttons available in the Sunfish Wizard.
...
The Wizard component allows you to include your custom buttons. You can define the desired buttons under the `WizardButtons` tag of the `SunfishWizard`.
...
<div style="text-align:center">
    <SunfishWizard  @bind-Value="@Value" Width="600px" Height="300px">
        <WizardSteps>
            <WizardStep Text="1">
                <Content>
                    <h2>Content for Wizard Step 1</h2>
        ...
        <WizardButtons>
            @{
                ...
                if (index > 0)
                {
                    <SunfishButton OnClick="@(() => Value = 0)">Go to first page</SunfishButton>
                    <SunfishButton OnClick="@(() => Value -= 1)">Previous</SunfishButton>
                }
                if (index != 2)
                {
                    <SunfishButton ButtonType="ButtonType.Button" ThemeColor="primary" OnClick="@(() => Value += 1)">Next</SunfishButton>
                    <SunfishButton ButtonType="ButtonType.Button" ThemeColor="primary" OnClick="@(() => Value = 2)">Go to last page</SunfishButton>
                }
            }
        </WizardButtons>
    </SunfishWizard>
...
````RAZOR
<SunfishWizard Class="disabled-stepper" />
...
````RAZOR
<SunfishWizard Class="hidden-stepper" />
...
@if (ShowWizard)
{
    <SunfishWizard @bind-Value="@CurrentWizardStep" OnFinish="@OnWizardFinish">
        <WizardSettings>
        ...
        <WizardButtons>
            @{
                ...
                {
                    <SunfishButton OnClick="@( () => PreviousClick(currentStepIndex) )">Back</SunfishButton>
                }
                if (currentStepIndex < 2)
                {
                    <SunfishButton ThemeColor="primary" OnClick="@( () => NextClick(currentStepIndex) )">Next</SunfishButton>
                }
                else
                {

                    <SunfishButton ThemeColor="primary" OnClick="@DoneClick">Done</SunfishButton>
                }
            }
        </WizardButtons>
    </SunfishWizard>
...
* [Live Demos: Wizard Buttons](https://demos.sunfish.dev/blazor-ui/wizard/buttons)
