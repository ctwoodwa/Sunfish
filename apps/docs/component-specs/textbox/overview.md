...
tags: sunfish,blazor,textbox,overview
published: True
position: 0
components: ["textbox"]
---
...
The <a href="https://sunfish.dev/blazor-ui/textbox" target="_blank">Blazor TextBox component</a> allows the user to enter a generic plain text message. 
...
1. Add the `<SunfishTextBox>` tag to a Razor file.
...
<SunfishTextBox @bind-Value="@StringValue" />
...
| `Password` | `bool` | When set to `true`, the HTML element renders `type="password"` so that the user input is hidden. You can find examples of validation and reveal buttons in the [Password Textbox demo](https://demos.sunfish.dev/blazor-ui/textbox/password) and the [Add Eye Icon to Reveal a TextBox Password](slug:textbox-kb-eye-reveal-password) article. |
...
<SunfishTextBox @ref="@TextBoxRef"
                @bind-Value="@TextBoxValue"
                Width="200px" />

@code {
    private SunfishTextBox TextBoxRef { get; set; }
...
<SunfishTextBox Placeholder="john@smith.com" Title="write your email here"
...
<SunfishButton OnClick="@ChangeValue">Change TextBox Value</SunfishButton>
...
* [Live Demo: TextBox](https://demos.sunfish.dev/blazor-ui/textbox/overview)
* [TextBox API Reference](slug:Sunfish.Blazor.Components.SunfishTextBox)
