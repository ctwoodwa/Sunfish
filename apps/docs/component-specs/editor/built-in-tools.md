...
tags: sunfish,blazor,built-in,tools
published: True
position: 40
components: ["editor"]
---
...
* `Tool Type` - a tool can be a [button](slug:Sunfish.Blazor.Components.Editor.ButtonTool), a [dropdown list](slug:Sunfish.Blazor.Components.Editor.DropDownListTool) or a [color picker](slug:Sunfish.Blazor.Components.Editor.ColorTool). Each of these three types exposes some customization options. See the examples for the [color tools](#color-tool-customization), [font tools](#font-tool-customization) and the [Format tool](#format-tool-customization). Only *buttons* can be added to tool groups in the [toolbar](slug:editor-toolbar).
...
````RAZOR
@using Sunfish.Blazor.Components.Editor
@* Avoid ambiguous reference with SVG icons *@
@using EditorNS = Sunfish.Blazor.Components.Editor;

<SunfishButton OnClick="@InsertParagraph">Insert Paragraph in the Editor</SunfishButton>

<SunfishEditor @ref="EditorRef"
               Tools="@EditorTools"
               @bind-Value="@EditorValue">
</SunfishEditor>

@code {
    private SunfishEditor EditorRef { get; set; }
...
| --- | --- | --- |
| `Colors` | `IEnumerable<string>`. | The list of available colors to set from the Color tool. You can provide a member of [`ColorPalettePresets`](slug:Sunfish.Blazor.ColorPalettePresets), or a custom list of [RGB(A) or HEX colors in different supported formats](slug:colorpicker-overview#supported-value-formats). |
...
<SunfishEditor Tools="@EditorTools"
               @bind-Value="@EditorValue">
</SunfishEditor>
...
The [`FontFamily`](slug:Sunfish.Blazor.Components.Editor.FontFamily) and [`FontSize`](slug:Sunfish.Blazor.Components.Editor.FontSize) tools have a `Data` property that accepts a `List<EditorDropDownListItem>`. Use it to customize the available options in these dropdowns. You can also change the dropdown label via `DefaultText`.
...
<SunfishEditor @bind-Value="@EditorValue"
               Tools="@EditorTools">
</SunfishEditor>
...
The [`Format` tool exposes a `Data` property](slug:Sunfish.Blazor.Components.Editor.Format) that accepts a `List<EditorDropDownListItem>`. Use it to reduce or reorder the items in the dropdown list.
...
<SunfishButton OnClick="@InsertHr">Insert hr</SunfishButton>
<SunfishButton OnClick="@BoldText">Create bold text</SunfishButton>
<SunfishButton OnClick="@InsertTable">Insert Table</SunfishButton>
<SunfishButton OnClick="@InsertImage">Insert Image</SunfishButton>
<SunfishButton OnClick="@InsertInlineText">Insert Inline Text</SunfishButton>

<SunfishEditor @ref="@TheEditor" @bind-Value="@TheContent"></SunfishEditor>

@code{
    SunfishEditor TheEditor { get; set; }
    ...
    {
        await TheEditor.ExecuteAsync(new ImageCommandArgs("https://demos.sunfish.dev/blazor-ui/images/articles/1-220x140.png", "the alt text", "220px", "140px"));
    }
