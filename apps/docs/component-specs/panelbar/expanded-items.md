...
description: Expand Items in the Sunfish PanelBar.
slug: panelbar-expand-items
tags: sunfish,blazor,panelbar,expand,items
published: True
position: 5
components: ["panelbar"]
---
...
You can expand a single or multiple items in the Sunfish PanelBar.
...
You can use the ExpandMode to allow the user to expand one or more items at a time. To control that set the `ExpandMode` parameter of the `<SunfishPanelBar>` to a member of the `PanelBarExpandMode` enum:
...
<SunfishButton OnClick="@ExpandItems">Expand First and Second Item</SunfishButton>
<SunfishButton OnClick="@CollapseItems">Collapse the Expanded Items</SunfishButton>
