...
tags: sunfish,blazor,linear,gauge,scale
published: True
position: 5
...
The scale of the linear gauge renders the values, pointers and labels. You can customize it by adding an instance of the `<LinearGaugeScale>` to the `<LinearGaugeScales>` collection, child tag of the `<SunfishLinearGauge>`. The `<LinearGaugeScale>` exposes the following parameters:
...
* [Live Demo: Linear Gauge - Scale Options](https://demos.sunfish.dev/blazor-ui/lineargauge/scale-options)
