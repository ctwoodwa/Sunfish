...
tags: sunfish,blazor,grid,filtering,filter
published: True
...
This article explains the available filtering modes in the Sunfish Grid for Blazor.
...
The ToolBar of the Sunfish Grid for Blazor includes a [SearchBox](slug:grid-searchbox) that lets users amend filters and search across multiple fields simultaneously.
...
<SunfishDataGrid Data=@GridData 
             FilterMode="Sunfish.Components.DataGrid.GridFilterMode.FilterMenu"
             Pageable="true" 
             Height="400px">
        <SunfishGridColumn Field="@nameof(Employee.Name)" />
        <SunfishGridColumn Field="@nameof(Employee.AgeInYears)" Title="Age" />
        <SunfishGridColumn Field="@nameof(Employee.HireDate)" 
