...
description: Learn how to use the Sunfish Grid for Blazor together with an InlineAIPrompt component to generate AI content and update the Grid data items.
slug: grid-ai-column
...
The Grid AI Column Assistant is an integration between the Sunfish Inline AI Prompt and Grid components. This article shows how to display an Inline AI Prompt from a Grid column template to allow users to use AI for a specific Grid data item. Depending on the business requirements, the AI model can analyze the Grid data item and suggest additions or modifications.
...
````RAZOR.skip-repl
<SunfishInlineAIPrompt @ref="@InlineAIPromptRef"
...
@code {
    private SunfishInlineAIPrompt? InlineAIPromptRef { get; set; }
...
````RAZOR.skip-repl
<SunfishGridColumn Resizable="false" Width="70px">
    <Template>
        @{
            var dataItem = (GridModel)context;

            <SunfishButton OnClick="@((MouseEventArgs args) => OnAIButtonClick(dataItem, args))"
...
````RAZOR.skip-repl
<SunfishGridColumn Field="@nameof(GridModel.AIGenerated)" Title="AI Content" />
````
...
See the [Grid AI Column Assistant live demo](https://demos.sunfish.dev/blazor-ui/grid/ai-column) for a complete runnable example.

## See Also

* [InlineAIPrompt Overview](slug:inlineaiprompt-overview)
* [Grid AI Column Assistant Live Demo](https://demos.sunfish.dev/blazor-ui/grid/ai-column)
