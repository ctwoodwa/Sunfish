...
slug: grid-overview
tags: sunfish,blazor,grid,datagrid,overview
published: True
hideCta: True
position: 0
components: ["grid"]
...
This article provides a quick introduction to get your first <a href="https://demos.sunfish.dev/blazor-ui/grid/overview" target="_blank">Blazor data grid component</a> up and running in a few seconds. There is a video tutorial and a list of the key features.
...
The Sunfish Blazor Data Grid provides a comprehensive set of ready-to-use features that cover everything from paging, sorting, filtering, editing, and grouping to row virtualization, optimized data reading, keyboard navigation, and accessibility support.

The Sunfish Blazor grid is built on native Blazor from the ground up, by a company with a long history of making enterprise-ready Grids. This results in a highly customizable Grid that delivers lighting fast performance.
...
1. Add [`SunfishGridColumn`](slug:components/grid/columns/bound) instances directly inside `<SunfishDataGrid>`. Each column `Field` should point to the model property to display. Use `nameof()` or the plain field name. Define user-friendly column `Title`s or [`DisplayFormat` for numeric and date values](slug:grid-columns-displayformat).
...
* Introduction to the Sunfish Blazor Grid
* Add SunfishRootComponent to MainLayout
* Add a Blazor Grid
* Configure columns
...
The following table lists Grid parameters, which are not discussed elsewhere in the component documentation. Check the [Grid API Reference](slug:Sunfish.Components.DataGrid.SunfishDataGrid-1) for a full list of parameters, methods and events.
