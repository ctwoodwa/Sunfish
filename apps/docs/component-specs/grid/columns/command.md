...
tags: sunfish,blazor,grid,column,command
published: True
position: 5
components: ["grid"]
---
...
To define it, add a `SunfishGridCommandColumn` inside `<SunfishDataGrid>`. The command column takes a collection of `GridCommandButton` instances that invoke the commands. It also provides the data item `context` and a `Title` property to set its header text.
...
* You can customize the appearance of the `GridCommandButton` by applying the [appearance attributes available for the SunfishButton](slug:button-appearance).
...
````RAZOR
<SunfishGridCommandColumn Context="dataItem">
...
<SunfishDataGrid Data=@GridData EditMode="@GridEditMode.Inline" OnUpdate="@MyOnUpdateHandler"
             Pageable="true" PageSize="15" Height="500px">
        <SunfishGridColumn Field=@nameof(SampleData.ID) Editable="false" Title="Employee ID" />
...
## See Also
  ...
  * [Live Demo: Grid Command Column](https://demos.sunfish.dev/blazor-ui/grid/editing-inline)
