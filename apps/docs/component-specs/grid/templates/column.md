...
tags: sunfish,blazor,grid,templates,column,cell
published: True
position: 5
components: ["grid"]
---
...
To define a column template, use a `<Template>` tag inside the `<SunfishGridColumn>` tag. The Grid uses the defined `Template` to show the "view" representation of all cells in that column. This also includes cells from [columns that are marked as `Editable="false"`](slug:components/grid/columns/bound#data-operations), while the cells' parent row is in [inline edit mode](slug:grid-editing-inline).
...
````RAZOR
<SunfishDataGrid Data="@GridData" Height="400px">
        <SunfishGridColumn Field="@(nameof(FoodItem.Id))" Title="Image">
            <Template>
                @{
                    var item = (FoodItem)context;
                    <img src="@($"https://demos.sunfish.dev/blazor-ui/images/{item.Id}.jpg")"
                         alt="Image of @item.Name" />
                }
            </Template>
        </SunfishGridColumn>
        <SunfishGridColumn Field="@(nameof(FoodItem.Name))">
            <Template>
                Food item name:
                <br />
        ...
        <SunfishGridColumn Field="@nameof(FoodItem.BestBefore)" Title="Date - Default format">
        </SunfishGridColumn>
        <SunfishGridColumn Field="@nameof(FoodItem.BestBefore)" Title="Date - Custom format string">
            <Template>
                @((context as FoodItem).BestBefore.ToString("dd MMM yyyy"))
            </Template>
        </SunfishGridColumn>
        <SunfishGridColumn Field="@nameof(FoodItem.Organic)">
            <Template>
                @{
                    var item = (FoodItem)context;
                }
                Read-only Checkbox:
                <SunfishCheckBox @bind-Value="@item.Organic" Enabled="false" />
                <br />
                or Icon:
                <SunfishSvgIcon Icon="@( item.Organic ? SvgIcon.CheckboxChecked : SvgIcon.Checkbox )" />
            </Template>
        </SunfishGridColumn>
</SunfishDataGrid>
