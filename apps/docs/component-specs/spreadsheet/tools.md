...
The Sunfish Blazor Spreadsheet organizes all its tools in a tool set with one or more tool set items. This article describes the built-in Spreadsheet tools and how to define custom tools.
...
The Spreadsheet uses Sunfish Blazor components to create its built-in tool sets and tools. You can add, remove, and reorder tools in the built-in tool sets. You can also customize the exposed tool properties.

The example below shows how to:

* Get the **File** and **Home** tool set items. The built-in tool set items are distinguishable by their index or `Title`. The title can change in localized applications, so you may need to use the [`ISunfishStringLocalizer` service](slug:globalization-localization) to search for the localized `Title` string.
...
````RAZOR
@using Sunfish.Blazor.Components.Spreadsheet
@using Sunfish.Blazor.Components.Spreadsheet.ToolBar.ToolTypes
@using Sunfish.Blazor.Resources
@using Sunfish.Blazor.Services

@* Needed to find the built-in Home tool by its localized title if the application is using more than one language *@
@* @inject ISunfishStringLocalizer Localizer *@

<SunfishSpreadsheet Tools="@DefaultToolsWithCustomizations">
</SunfishSpreadsheet>
...
<SunfishSpreadsheet Tools="@SpreadsheetTools">
</SunfishSpreadsheet>
...
@code {
    ...
    private RenderFragment CustomToolFragment =>
        @<SunfishButton Icon="@SvgIcon.FileExcel"
...
* [Live Demo: Spreadsheet Tools](https://demos.sunfish.dev/blazor-ui/spreadsheet/tools)
