...
The Sunfish Blazor Spreadsheet fires events that are related to different user actions. This article describes all events and event arguments.
...
````RAZOR
<p><label><SunfishCheckBox @bind-Value="@ShouldCancelEvents" /> Cancel Spreadsheet Events</label></p>

<p>Spreadsheet Event Log: @( new MarkupString(SpreadSheetEventLog) )</p>

<SunfishSpreadsheet OnDownload="@OnSpreadsheetDownload"
...
@code {
    ...
    private void OnSpreadsheetDownload(SpreadsheetDownloadEventArgs args)
    {
        ...
        args.FileName = $"sunfish-spreadsheet-{now.ToString("HH-mm-ss")}";
