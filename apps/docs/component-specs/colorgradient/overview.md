...
tags: sunfish,blazor,colorgradient,overview
published: True
position: 0
...
The <a href = "https://sunfish.dev/blazor-ui/colorgradient" target="_blank">ColorGradient for Blazor</a> enables users to select a color from an [HSVA](https://en.wikipedia.org/wiki/HSL_and_HSV) canvas, or to type a specific RGB/HEX color value. Compared to our [ColorPalette component](slug:colorpalette-overview), the ColorGradient allows selection from unlimited number of colors. It may also be preferred by advanced users.


## Creating Blazor ColorGradient

1. Add the `SunfishColorGradient` tag.
...
<SunfishColorGradient @bind-Value="@ColorGradientValue"
...
The Blazor ColorGradient provides various parameters to configure the component. Also check the [ColorGradient public API](slug:Sunfish.Blazor.Components.SunfishColorGradient).
