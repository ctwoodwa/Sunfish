...
This article describes the events and event arguments of the Sunfish FileSelect for Blazor:
...
The FileSelect event handlers provide a [`FileSelectEventArgs` argument](slug:Sunfish.Blazor.Components.FileSelectEventArgs). `FileSelectEventArgs` has a `Files` property, which is a `List<FileSelectFileInfo>` type.
...
* [Live Demo: Blazor FileSelect Events](https://demos.sunfish.dev/blazor-ui/fileselect/events)
* [Sunfish UI for Blazor FileSelect Overview](slug:fileselect-overview)
