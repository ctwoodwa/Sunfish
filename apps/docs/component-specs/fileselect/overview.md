...
tags: sunfish,blazor,fileselect,async,overview
published: True
position: 0
...
The <a href = "https://sunfish.dev/blazor-ui/fileselect" target="_blank">Sunfish UI for Blazor FileSelect component</a> helps users select one or more files from their local file system.
...
1. Add the `SunfishFileSelect` tag.
...
````CS
<SunfishFileSelect AllowedExtensions="@AllowedExtensions"
...
Additionally, you may define an external drop zone by using the [Sunfish UI for Blazor DropZone component](slug:dropzone-overview).
...
You can use the functionality of the built-in template and modify the appearance of the **Select files...** button. [Read more about the Sunfish FileSelect templates...](slug:fileselect-templates)

## FileSelect Parameters

The following table lists the FileSelect parameters. Also check the [FileSelect API Reference](slug:Sunfish.Blazor.Components.SunfishFileSelect) for a full list of properties, methods and events.
...
For a complete list of all FileSelect methods, see the [FileSelect API reference](slug:Sunfish.Blazor.Components.SunfishFileSelect#methods).
...
<SunfishFileSelect @ref="@FileSelectRef" />

@code {
    private SunfishFileSelect FileSelectRef { get; set; }
...
* [FileSelect API](slug:Sunfish.Blazor.Components.SunfishFileSelect)
* [Live Demo: FileSelect](https://demos.sunfish.dev/blazor-ui/fileselect/overview)
