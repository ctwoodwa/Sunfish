...
<SunfishListBox @ref="@ListBoxRef"
    ...
    <ItemTemplate>
        <SunfishSvgIcon Icon="@context.Icon" />
        @context.FirstName <strong>@context.LastName</strong>
    </ItemTemplate>
    <NoDataTemplate>
        <div>
            No items?..<br />
            <SunfishButton OnClick="@AddItems">Add Some</SunfishButton>
        </div>
    </NoDataTemplate>
</SunfishListBox>

@code {
    private SunfishListBox<Person> ListBoxRef { get; set; } = null!;
...
* [Live Demo: ListBox Templates](https://demos.sunfish.dev/blazor-ui/listbox/templates)
