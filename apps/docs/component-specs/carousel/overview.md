...
tags: sunfish,blazor,carousel,overview
published: True
position: 0
components: ["carousel"]
---
...
The <a href = "https://sunfish.dev/blazor-ui/carousel" target="_blank">Carousel for Blazor</a> is an interactive component that allows users to browse a collection of content items (pages) one at a time. The Carousel template supports random web content, although it is most often used to display images.

## Creating Blazor Carousel

1. Use the `SunfishCarousel` tag to add the component to your razor page.
...
<SunfishCarousel Data="@CarouselData"
...
The following table lists Carousel parameters. Check the [Carousel API Reference](slug:Sunfish.Blazor.Components.SunfishCarousel-1) for a full list of properties, methods and events.
...
The table below lists the Carousel methods. Also consult the [Carousel API](slug:Sunfish.Blazor.Components.SunfishCarousel-1).
...
````RAZOR
<SunfishCarousel @ref="@CarouselRef" .../>

@code{
    private SunfishCarousel<MyModel> CarouselRef;
}
````
...
* [Carousel Live Demo](https://demos.sunfish.dev/blazor-ui/carousel/overview)
* [Carousel API Reference](slug:Sunfish.Blazor.Components.SunfishCarousel-1)
