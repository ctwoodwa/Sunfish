...
description: Discover the Sunfish UI for Blazor DropZone, learn how to create the component, integrate it with the Upload and FileSelect controls, use its templaes, parameters, style its appearance, and explore its examples.
slug: dropzone-overview
tags: sunfish,blazor,dropzone,external,drag,drop,file,overview
published: True
position: 0
components: ["dropzone"]
---
...
The <a href="https://sunfish.dev/blazor-ui/dropzone" target="_blank">Sunfish UI for Blazor DropZone component</a> allows you to declare an external drop zone for an existing [FileSelect](slug:fileselect-overview) or [Upload](slug:upload-overview) component.
...
1. Add the DropZone to your Razor page by using the `SunfishDropZone` tag.
...
````RAZOR
<SunfishDropZone Id="@DropZoneId" NoteText="@NoteText" />

<SunfishFileSelect DropZoneId="@DropZoneId" AllowedExtensions="@AllowedExtensions" />
...
The following table lists DropZone parameters. Check the [DropZone API Reference](slug:Sunfish.Blazor.Components.SunfishDropZone) for a full list of properties, methods and events.
...
| `DragOverClass` | `string` | The CSS class that will be rendered on the main wrapping element of the DropZone when a file is dragged over it. Use it for [conditionally styling the component during a dragover action](https://demos.sunfish.dev/blazor-ui/dropzone/overview). |
...
* [Live Demo: Blazor DropZone Overview and Key Features](https://demos.sunfish.dev/blazor-ui/dropzone/overview)
