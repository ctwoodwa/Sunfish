...
slug: badge-overview
tags: sunfish,blazor,badge,navbar
published: True
position: 0
components: ["badge"]
---
...
The <a href = "https://sunfish.dev/blazor-ui/badge" target="_blank">Blazor Badge component</a> is a visual element that displays additional information, such as a status indicator, notification icon, short text, and others.
...
1. Add the `<SunfishBadge>` tag to a container that the Badge will attach to.
1. Use the Badge `ChildContent` to add content.
1. (optional) Customize the [appearance](slug:badge-appearance) and fine-tune the [positioning and alignment](slug:badge-position-alignment) of the Sunfish Blazor Badge.

>caption Basic configuration of the Blazor Badge

````RAZOR
<SunfishButton>
    <SunfishBadge VerticalAlign="@BadgeVerticalAlign.Bottom">12</SunfishBadge>
    Notifications
</SunfishButton>
````
...
The Blazor Badge provides various parameters that allow you to configure the component. Also check the [Badge API Reference](slug:Sunfish.Blazor.Components.SunfishBadge) for a full list of properties.
...
| `FillMode` | `Sunfish.Blazor.ThemeConstants.Badge.FillMode.Solid` | Controls how the Badge is filled. |
| `Rounded` | `Sunfish.Blazor.ThemeConstants.Badge.Rounded.Full` | Defines how rounded the borders of the Badge are. |
| `Size` | `Sunfish.Blazor.ThemeConstants.Badge.Size.Medium` | Sets the size of the Badge. |
| `ThemeColor` | `Sunfish.Blazor.ThemeConstants.Badge.ThemeColor.Primary` | Adjusts the background color of the Badge. |
...
* [Live Badge Demos](https://demos.sunfish.dev/blazor-ui/badge/overview)
* [Badge API Reference](slug:Sunfish.Blazor.Components.SunfishBadge)
