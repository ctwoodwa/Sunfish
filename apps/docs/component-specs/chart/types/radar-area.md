...
tags: sunfish,blazor,chart,radar,area
published: True
position: 0
components: ["charts"]
---
...
<SunfishChart>
    <ChartSeriesItems>
        <SunfishChartSeries Type="ChartSeriesType.RadarArea" Name="John Smith" Data="@series1Data">
        </SunfishChartSeries>
        <SunfishChartSeries Type="ChartSeriesType.RadarArea" Name="Jane Doe" Data="@series2Data">
        </SunfishChartSeries>
    </ChartSeriesItems>
    ...
    <ChartLegend Position="Sunfish.Blazor.ChartPosition.Right">
    </ChartLegend>
</SunfishChart>
...
You can control the color of the line itself separately by using the `Color` property of the nested `SunfishChartSeriesLine` tag. To see the line, set its `Width` parameter to a value larger than `0`.
...
You can render the lines between the points with different styles. The supported styles can be set via the `Style` property of the child `ChartSeriesLine` tag - it takes a member of `Sunfish.Blazor.ChartSeriesLineStyle` enum:
...
## See Also
  ...
  * [Live Demo: Radar Area Chart](https://demos.sunfish.dev/blazor-ui/chart/radar-area-chart)
