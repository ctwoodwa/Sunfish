...
tags: sunfish,blazor,gantt,chart,dependency,databind,data,databound
published: True
position: 5
...
The current implementation uses a simplified `DependsOnField` string parameter on `SunfishGantt` to specify a model property that contains the ID of the predecessor task. This provides basic finish-to-start dependency rendering without a separate dependency collection.
