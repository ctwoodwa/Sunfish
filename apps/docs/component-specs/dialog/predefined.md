...
description: Predefined dialogs for Blazor - popup message, alert, confirm, and prompt. Explore the Sunfish UI documentation.
slug: dialog-predefined
tags: sunfish,blazor,dialog,predefined,alert,confirm,prompt
published: true
position: 2
components: ["dialog"]
---
...
Sunfish UI for Blazor provides styled substitutes to the standard alert, confirm, and prompt dialogs of the browser. These Blazor popup messages match the theme of the components to make it obvious to the user that the modal dialog is coming from your application.


## Using Predefined Dialogs

To use the alert, confirm, and prompt dialogs (popup messages), receive a cascading parameter of type `Sunfish.Blazor.DialogFactory`. This object exposes the methods you can use to display the predefined dialogs.
...
### SunfishRootComponent Dependency

The `DialogFactory` cascading parameter can only be consumed inside child Razor components of the [`SunfishRootComponent`](slug:rootcomponent-overview#purpose). This is easily achieved in standard Blazor apps where the `SunfishRootComponent` resides inside the layout file.

* To show predefined dialogs in `MainLayout.razor`, [add the `SunfishRootComponent` in a parent layout](slug:rootcomponent-overview#using-sunfishrootcomponent).
* When [using UI for Blazor in ASP.NET apps](slug:sunfish-blazor-in-asp-net-app), an additional child component is needed.

If the `SunfishRootComponent` is not available, or if it's at the wrong place in the app, [the `DialogFactory` will trigger a `NullReferenceException`](slug:dialog-kb-dialogfactory-null).

### Using await

The `DialogFactory` methods must be awaited. Do not use them with [discard variables (`_`)](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/functional/discards). If you don't need to await the user response, then [use the `<SunfishDialog>` component](slug:dialog-overview) declaratively. 
...
<SunfishButton OnClick="@ShowAlert">Show Alert</SunfishButton>
<SunfishButton OnClick="@ShowAlertWithTitle">Show Alert with Custom Title</SunfishButton>
<SunfishButton OnClick="@ShowAlertWithTitleAndButton">Show Alert with Custom Title and Custom Button</SunfishButton>
...
<SunfishButton OnClick="@ShowConfirm">Show Confirm</SunfishButton>
<SunfishButton OnClick="@ShowConfirmWithTitle">Show Confirm with Custom Title</SunfishButton>
<SunfishButton OnClick="@ShowConfirmWithTitleAndButtons">Show Confirm with Custom Title and Custom Buttons</SunfishButton>
...
<SunfishButton OnClick="@ShowPrompt">Show Prompt</SunfishButton>
<SunfishButton OnClick="@ShowPromptWithTitle">Show Prompt with Custom Title</SunfishButton>
<SunfishButton OnClick="@ShowPromptWithTitleAndDefaultText">Show Prompt with Title and Default Input Text</SunfishButton>
<SunfishButton OnClick="@ShowPromptWithTitleDefaultTextAndButtons">Show Prompt with Title, Default Input Text and Custom Buttons</SunfishButton>
...
* [Live Demo: Predefined Dialogs](https://demos.sunfish.dev/blazor-ui/dialog/predefined-dialogs)
* [Using the `<SunfishDialog>` Component Declaratively](slug:dialog-overview)
