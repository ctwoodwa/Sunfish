...
page_title: Sunfish UI for Blazor Window Documentation | Window Accessibility Overview
description: "Get started with the Sunfish UI for Blazor Window and learn about its accessibility support for WAI-ARIA, Section 508, and WCAG 2.2."
tags: sunfish,blazor,accessibility,wai-aria,wcag,window
...
The illustrative example below shows the window resize action, achievable through the [Dialog](slug:dialog-overview) and window position change through a button. Sunfish UI for Blazor to offer a versatile API that allows users to trigger all functions programmatically or externally, meeting diverse accessibility requirements for any applications.
...
<div @onclick="@HandleWindowPosition" class="window-container">
    <SunfishWindow @bind-Visible="@WindowIsVisible"
        ...
        <WindowContent>
            <ul>
                <li>
                    ...
                    <SunfishSvgIcon Icon="@SvgIcon.ArrowsMove"></SunfishSvgIcon> icon and then
                    click anywhere on the screen.
                </li>
                <li>Click the "Resize Window" button and enter the new dimensions.</li>
            </ul>
            <SunfishButton AriaLabel="Open Resize Dialog" Class="resize-button" OnClick="@(() => DialogVisible = true)">Resize Window</SunfishButton>
        </WindowContent>
        <WindowActions>
    ...
    <SunfishDialog @bind-Visible="@DialogVisible"
                   Title="Resize"
                   Width="450px"
                   Height="220px">
        <DialogContent>
            New width:
            <div class="resize">
                <SunfishNumericTextBox AriaLabel="Set New Width" @bind-Value="NewWidth" Min="0" Format="## pixels" />
            </div>
            New height:
            <div class="resize">
                <SunfishNumericTextBox AriaLabel="Set New Height" @bind-Value="NewHeight" Min="0" Format="## pixels" />
            </div>
        </DialogContent>
        <DialogButtons>
            <SunfishButton AriaLabel="Confirm Resize" OnClick="@ApplyResize" ThemeColor="@ThemeConstants.Button.ThemeColor.Primary"><SunfishSvgIcon Icon="@SvgIcon.Check" /><span style="margin-left: 5px;">Resize</span></SunfishButton>
            <SunfishButton AriaLabel="Cancel Resize" OnClick="@(() => { DialogVisible = false; })"><SunfishSvgIcon Icon="@SvgIcon.Cancel" /><span style="margin-left: 5px;">Cancel</span></SunfishButton>
        </DialogButtons>
    </SunfishDialog>

@if (!WindowIsVisible)
{
    <SunfishButton AriaLabel="Open Window" OnClick="@( () => WindowIsVisible = !WindowIsVisible )">Open window</SunfishButton>
}
</div>
...
## See also
 * [Live demo: Window Accessibility](https://demos.sunfish.dev/blazor-ui/window/keyboard-navigation)
 * [Live demo: Window Overview](https://demos.sunfish.dev/blazor-ui/window/overview)
 * [Live demo: Blazor Accessibility Overview](https://docs.sunfish.dev/blazor-ui/accessibility/overview)