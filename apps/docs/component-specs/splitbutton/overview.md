...
tags: sunfish,blazor,splitbutton
published: True
position: 0
...
The <a href = "https://sunfish.dev/blazor-ui/split-button" target="_blank">SplitButton for Blazor</a> is a combination of a button and a dropdown. It provides a collection of related user actions in a compact interface. The SplitButton has one primary clickable action, which is always visible, and a list of secondary actions that are displayed in a dropdown when the user clicks on the arrow.
...
1. Add a `<SunfishSplitButton>` tag.
...
1. Set the `OnClick` parameter of the `<SunfishSplitButton>` tag. This will be the event handler for the primary action.
...
````RAZOR
<SunfishSplitButton OnClick="@OnReply">
...
>tip To learn more about the appearance, anatomy, and accessibility of the SplitButton, visit the [Progress Design System Kit documentation](https://sunfish.dev/design-system/docs/components/splitbutton/)—an information portal offering rich component usage guidelines, descriptions of the available style variables, and globalization support details.
...
The following table lists the SplitButton parameters, except those related to [built-in styling](slug:splitbutton-appearance) and [icons](slug:splitbutton-icons). Also check the [SplitButton API Reference](slug:Sunfish.Blazor.Components.SunfishSplitButton) for a full list of properties, methods and events.
...
````RAZOR
<SunfishSplitButton @ref="@SplitButtonRef" OnClick="@OnReply">
    <SplitButtonContent>Reply</SplitButtonContent>
    <SplitButtonItems>
        <SplitButtonItem OnClick="@OnReplyAll">Reply All</SplitButtonItem>
    </SplitButtonItems>
...
<SunfishButton OnClick="@FocusSplitButton">Focus SplitButton</SunfishButton>

Last action clicked: <strong> @LastActionClicked </strong>

@code {
    SunfishSplitButton SplitButtonRef { get; set; }
...
* [SplitButton API](slug:Sunfish.Blazor.Components.SunfishSplitButton)
* [Live Demo: SplitButton](https://demos.sunfish.dev/blazor-ui/splitbutton/overview)
