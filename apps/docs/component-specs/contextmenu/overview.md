...
description: Try now the Sunfish UI for Blazor Context Menu and learn more about its options for creating the component, binding it to data, changing its navigation views, customizing the component per target, and more.
slug: contextmenu-overview
tags: sunfish,blazor,context menu,overview
published: True
position: 0
...
The <a href="https://sunfish.dev/blazor-ui/context-menu" target="_blank">Blazor Context Menu</a> displays a contextual popup with flat or hierarchical data in a traditional menu-like structure. The component enables you to invoke commands while preserving the screen real estate.
...
1. Add the `SunfishContextMenu` tag and set its `Selector` parameter to a CSS selector that will match the elements to which you want to attach the Context Menu.
...
<SunfishContextMenu Selector=".context-menu-target" Data="@MenuItems"                       
...
The following table lists Context Menu parameters, which are not related to other features on this page. For the full list of properties, methods, and events, see the [Context Menu API reference documentation](slug:Sunfish.Blazor.Components.SunfishContextMenu-1).
...
To use the [Blazor Context Menu methods](slug:Sunfish.Blazor.Components.SunfishContextMenu-1), add a reference to the component instance.
...
<SunfishContextMenu Data="@MenuItems" @ref="@TheContextMenu">
    <Template>
        @{
        ...
        <SunfishTextBox @bind-Value="@TextBoxValue" />
        <br />
        <SunfishButton OnClick="@HandleTextBoxReset">Reset textbox</SunfishButton>
        <SunfishButton OnClick="@(async () => await TheContextMenu.HideAsync())">Close</SunfishButton>
    </Template>
</SunfishContextMenu>
...
@code {
    ...
    SunfishContextMenu<ContextMenuItem> TheContextMenu { get; set; }
...
* [Binding the Sunfish UI for Blazor Context Menu to Data](slug:contextmenu-data-binding-overview)
...
* [Live Demo: Overview of the Blazor Context Menu](https://demos.sunfish.dev/blazor-ui/contextmenu/overview)
* [Context Menu API Reference](slug:Sunfish.Blazor.Components.SunfishContextMenu-1)
