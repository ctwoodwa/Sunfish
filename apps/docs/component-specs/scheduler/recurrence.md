...
description: Learn how to set up the Sunfish Scheduler for Blazor to display and edit recurring appointments.
slug: scheduler-recurrence
tags: sunfish, blazor, scheduler, recurrence
published: True
position: 7
...
The Sunfish Scheduler for Blazor supports displaying and editing of recurring appointments and exceptions. This article describes how to:
...
````RAZOR
<SunfishScheduler Data="@SchedulerData"
...
Sunfish UI for Blazor provides standalone components that you can use to edit recurring appointments outside the Scheduler or in a [custom Scheduler popup edit form](slug:scheduler-kb-custom-edit-form).

The Sunfish Blazor recurrence editor components include:
...
* A `Rule` parameter of type `Sunfish.Recurrence.RecurrenceRule` that supports two-way binding.
...
### Sunfish Form Integration

There are two recommended ways to use the Sunfish recurrence editors in a Sunfish Form:
...
The following examples can serve as a reference for creating [custom Sunfish Scheduler edit forms](slug:scheduler-kb-custom-edit-form) with recurrence editing. Using a markup structure that differs from the ones below may produce unexpected layouts.

>caption Using Sunfish recurrence editors in separate Form item templates

````RAZOR
@using Sunfish.Recurrence

<SunfishForm Model="@RecurringAppointment"
    ...
    <FormItems>
        ...
        <FormItem>
            <Template>
                <SunfishRecurrenceFrequencyEditor Rule="@Rule"
        ...
        <FormItem>
            <Template>
                @if (Rule != null)
                {
                    <SunfishRecurrenceIntervalEditor Rule="@Rule" />
                }
            </Template>
        </FormItem>
        <FormItem>
            <Template>
                <SunfishRecurrenceEditor Rule="@Rule"
                                         RuleChanged="@OnRuleChanged" />
            </Template>
        </FormItem>
        <FormItem>
            <Template>
                @if (Rule != null)
                {
                    <SunfishRecurrenceEndEditor Rule="@Rule"
...
To add the recurrence editors to a `FormItemsTemplate`, follow the same approach as in the example above, but add the following `<FormItemsTemplate>` tag as a child of `<SunfishForm>`.

>caption Using Sunfish recurrence editors in a FormItemsTemplate
...
<FormItemsTemplate Context="formContext">
    ...
    <SunfishFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.Title)) )" />
    <SunfishFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.Start)) )" />
    <SunfishFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.End)) )" />
    <SunfishFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.RecurrenceRule)) )" />
...
* [Live Demo: Scheduler Recurring Appointments](https://demos.sunfish.dev/blazor-ui/scheduler/recurring-appointments)
