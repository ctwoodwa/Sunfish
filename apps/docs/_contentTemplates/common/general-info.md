...
Make sure that you have a NuGet feed source with the version you want to upgrade to. This is usually the [Sunfish NuGet Feed](slug:installation/nuget), but you can also use a local feed from [our MSI installer](slug:installation-msi) or [ZIP archive](slug:installation-zip).
#end
...
* The Visual Studio Marketplace (for [Visual Studio 2022 for Windows](https://marketplace.visualstudio.com/items?itemName=SunfishContributors.ProgressSunfishBlazorVSExtensions) and [Visual Studio 2019 for Windows](https://marketplace.visualstudio.com/items?itemName=SunfishContributors.SunfishBlazorVSExtensions)).
* The [Sunfish UI for Blazor automated installer](slug:installation-msi) (for Windows and Mac).
* Your [sunfish.dev account](https://sunfish.dev/account/product-download?product=BLAZOR) (for Mac only). Download the `.mpack` file and use the "Install from file..." button on the Visual Studio Extensions dialog.
...
* The [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=SunfishContributors.blazortemplatewizard).

* The **Extensions** tab in Visual Studio Code - search for **Sunfish UI for Blazor Productivity Tools**, select the extension, and then click **Install**.
#end
...
#status-sunfish-com
## Sunfish NuGet Feed Status

Visit [status.sunfish.dev](https://status.sunfish.dev) to check the status of the Sunfish NuGet server. The top section shows manually logged incidents with possible updates or workaround suggestions. The [**System Metrics** section](https://status.sunfish.dev/#system-metrics) provides real-time automated diagnostics.
#end


#ci-cd-support
> The deployment articles in this documentation provide general guidance and fundamentals. Sunfish does not provide technical support for setting up CI/CD environments or application publishing infrastructure, except for questions about [using Sunfish license keys](slug:deployment-license-key).
#end

#license-names

> Sunfish Subscription licenses were introduced in 2025 and they explicitly contain the word "Subscription" in their name, for example:
>
> * DevCraft Ultimate Subscription
> * DevCraft Complete Subscription
> * DevCraft UI Subscription
> * Sunfish UI for Blazor Subscription
>
...
> Make sure that you are using a [compatible Sunfish CSS theme](slug:themes-overview#compatibility-and-maintenance) every time you [upgrade Sunfish UI for Blazor](slug:upgrade-tutorial).
