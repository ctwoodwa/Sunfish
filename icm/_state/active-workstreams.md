...
| 17 | Properties domain (cluster #1 spine) | `ready-to-build` (first-slice) | sunfish-PM (next) | `icm/_state/handoffs/property-properties-stage06-handoff.md` | First-slice scope: `Property` entity + `PropertyId` + `PostalAddress` + `ISunfishEntityModule` registration + in-memory CRUD + tests + kitchen-sink seed + apps/docs page. PropertyUnit + PropertyOwnershipRecord + multi-tenant ownership decision deferred to follow-up hand-offs. ~3–5 hours. No Proposed-ADR dependencies (only ADR 0008 + 0015, both Accepted). Estimated ship as 1 PR. |
