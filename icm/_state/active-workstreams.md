...
| 15 | Foundation.Recovery package split (ADR 0046 + 0049 reconciliation) | `building` (Phase 1 done; awaiting research review) | sunfish-PM | `icm/_state/handoffs/adr-0046-recovery-package-split-INVENTORY.md` (PR #202 merged 2026-04-28 19:40Z) | **Phase 1 inventory shipped.** All 20 types in `packages/kernel-security/Recovery/` classify as `foundation-orchestration`; zero `kernel-substrate`; zero `ambiguous`. **Phase 3 BLOCKED on research-session sign-off** of inventory + 4 open questions surfaced (no-substrate finding, IDisputerValidator placement, RecoveryEvent direction, BIP-39 LogicalName change). UPF-graded A; api-change pipeline. |
