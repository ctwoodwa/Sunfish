...
/// canonical envelope; clusters that emit events early carry a local
/// copy until <c>foundation-events</c> lands, at which point each
/// cluster's first-emission migration PR re-namespaces to the
/// canonical home.
/// </para>
/// <para>
/// <b>Idempotency-key convention</b> for periods events:
/// <c>{eventType}|{tenantId}|{periodId}|{newStatus}</c>. Deterministic;
/// same (tenant, period, transition) always produces the same key so
/// retries are idempotent at the consumer side.
/// </para>
/// </remarks>
/// <typeparam name="TPayload">The event payload type (e.g., <c>PeriodSoftClosed</c>, <c>PeriodLocked</c>).</typeparam>
public sealed record DomainEventEnvelope<TPayload>
{
    ...
    /// catalog (§3.1), e.g., <c>"Financial.PeriodSoftClosed"</c>.
    ...
    /// convention. Consumers use this to discard duplicate deliveries
    /// without checking payload equality.
    /// </summary>
    public required string IdempotencyKey { get; init; }

    /// <summary>Optional — id of the prior event in a causal chain (e.g., "the SoftClose that triggered this Lock").</summary>
