...
public class SunfishFreshnessBadgeA11yTests : IClassFixture<SunfishFreshnessBadgeA11yTests.Ctx>
{
    private readonly Ctx _ctx;

    public SunfishFreshnessBadgeA11yTests(Ctx ctx) => _ctx = ctx;

    [Fact]
    public async Task SunfishFreshnessBadge_RecentSync_HasNoModeratePlusAxeViolations()
    {
        var rendered = _ctx.Bunit.Render<SunfishFreshnessBadge>(p => p
            .Add(c => c.LastSyncedAt, DateTimeOffset.UtcNow.AddSeconds(-30)));
        ...
        try
        {
            ...
            Assert.True(moderatePlus.Count == 0,
                $"SunfishFreshnessBadge surfaced {moderatePlus.Count} moderate+ axe violation(s): " +
