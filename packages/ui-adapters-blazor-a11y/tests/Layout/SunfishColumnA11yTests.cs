...
public class SunfishColumnA11yTests : IClassFixture<SunfishColumnA11yTests.Ctx>
{
    private readonly Ctx _ctx;

    public SunfishColumnA11yTests(Ctx ctx) => _ctx = ctx;

    [Fact]
    public async Task SunfishColumn_WithSpan_HasNoModeratePlusAxeViolations()
    {
        var rendered = _ctx.Bunit.Render<SunfishColumn>(p => p
            .Add(c => c.Span, 6)
        ...
        try
        {
            ...
            Assert.True(moderatePlus.Count == 0,
                $"SunfishColumn surfaced {moderatePlus.Count} moderate+ axe violation(s): " +
