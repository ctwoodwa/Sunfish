...
public class SunfishIconA11yTests : IClassFixture<SunfishIconA11yTests.Ctx>
{
    private readonly Ctx _ctx;

    public SunfishIconA11yTests(Ctx ctx) => _ctx = ctx;

    [Fact]
    public async Task SunfishIcon_NamedDefault_HasNoModeratePlusAxeViolations()
    {
        var rendered = _ctx.Bunit.Render<SunfishIcon>(p => p
            .Add(c => c.Name, "save"));
        ...
        try
        {
            ...
            Assert.True(moderatePlus.Count == 0,
                $"SunfishIcon surfaced {moderatePlus.Count} moderate+ axe violation(s): " +
