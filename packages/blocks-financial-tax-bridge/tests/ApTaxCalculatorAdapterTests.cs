...
public sealed class ApTaxCalculatorAdapterTests
{
    ...
    private static (ApTaxCalculatorAdapter Adapter, FakeTaxCalculationService Canonical) Build()
    {
        ...
        return (new ApTaxCalculatorAdapter(fake, opts), fake);
    }
    ...
    {
        var (a, fake) = Build();
        Assert.Equal(0m, await a.CalculateAsync(null, 250m, Txn));
    ...
    {
        var (a, fake) = Build();
        Assert.Equal(0m, await a.CalculateAsync("   ", 250m, Txn));
        Assert.Null(fake.LastInput);
    }
    ...
    {
        var (a, fake) = Build();
        fake.Next = OkResult(18.75m);
        Assert.Equal(18.75m, await a.CalculateAsync("VA-WHOLESALE", 250m, Txn));
    }
    ...
    public async Task Returns_Zero_When_Canonical_Errors()
    {
        ...
        Assert.Equal(0m, await a.CalculateAsync("MISSING", 250m, Txn));
    }
    ...
    public async Task Forwards_TransactionDate_Verbatim()
    {
        ...
        await a.CalculateAsync("VA-WHOLESALE", 250m, date);
    ...
    public async Task Forwards_CancellationToken()
    {
        ...
        cts.Cancel();
        await a.CalculateAsync("VA-WHOLESALE", 250m, Txn, cts.Token);
    ...
    {
        var (a, fake) = Build();
        await a.CalculateAsync("VA-WHOLESALE", 250m, Txn);
