...
public partial class SunfishDataGrid<TItem> : SunfishComponentBase
{
    ...
    // ── DI services ─────────────────────────────────────────────────────
    [Inject] private IDownloadService _downloadService { get; set; } = default!;
    ...
    internal bool IsBoundaryFrozenColumn(SunfishGridColumn<TItem> column)
    {
        ...
        var visible = _visibleColumns;
        var idx = visible.IndexOf(column);
        if (idx < 0) return false;

        if (column.FrozenPosition == GridColumnFrozenPosition.Start)
        {
            // Boundary = this is the last Start-frozen column in the visible list
            // (next column is non-locked, End-frozen, or out of bounds).
            var next = idx + 1 < visible.Count ? visible[idx + 1] : null;
            return next is null || !next.Locked || next.FrozenPosition == GridColumnFrozenPosition.End;
        }
        else // End
        {
            // Boundary = this is the leftmost End-frozen column
            // (prev column is non-locked, Start-frozen, or out of bounds).
            var prev = idx - 1 >= 0 ? visible[idx - 1] : null;
            return prev is null || !prev.Locked || prev.FrozenPosition == GridColumnFrozenPosition.Start;
        }
    }
