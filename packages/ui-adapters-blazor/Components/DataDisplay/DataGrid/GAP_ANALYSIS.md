...
- [x] B3.1 — Add `AllowColumnReorder` bool parameter on grid (default: false)
- [x] B3.2 — Add `Reorderable` bool parameter to `SunfishGridColumn` (default: true when grid allows)
- [x] B3.3 — Add `OrderIndex` (int?) property to `SunfishGridColumn` for persistence
- [x] B3.4 — JS: `installColumnReorder(handle)` — HTML5 drag-and-drop with visual clone `<th>` positioned absolutely
- [x] B3.5 — .NET: cancelable `OnColumnReordering` event fires before move; `OnColumnReordered(OldIndex, NewIndex)` fires after
- [x] B3.6 — Remove column from `_columns` and re-insert at target index; call `SetOrderIndex` on all columns
- [x] B3.7 — Persist column order in `GridState.ColumnStates`
