...
**Loader dispatch (required — read before implementing Checks 1–3).** Each vector has a `kind` field of `"HELLO"`, `"HEARTBEAT"`, or `"CONFIRM_TRANSCRIPT"`. The loader MUST dispatch on `kind` to select the correct field set: HELLO and HEARTBEAT vectors use `expected_signable_hex` + `expected_signature_hex`; CONFIRM_TRANSCRIPT vectors use `expected_input_hex` + `expected_sha256_hex`. A vector with an unrecognized `kind` MUST fail loudly (reserved for future amendments). Do not attempt to read `expected_signable_hex` from a CONFIRM vector or vice versa.

**Hex comparison rule (applies to all three checks).** All expected-output comparisons MUST be performed on the **decoded byte arrays**, not on the hex strings. Decode both expected and computed hex to raw bytes, then assert byte-for-byte equality. The JSON document emits lowercase, no-prefix hex; loaders that emit uppercase or padded hex (e.g. C# `ToString("X2")`, Java `String.format("%X")`) MUST normalize before committing their output but MUST decode-and-compare-bytes in the assertions themselves. String equality on the hex representation is implementation-specific and NOT part of the conformance contract.

**Length-prefix endianness rule (Check 1).** All `uint32BE` length prefixes are **big-endian, 4 bytes** (per §A1 §A4; "uint32BE" means network byte order). A length-prefix of `15` is encoded as `00 00 00 0f`; a length of `63` is `00 00 00 3f`. Sample: V5 produces a run `…00 00 00 3f 78 78 78…` (the `0x3f` immediately precedes 63 bytes of `0x78`). A little-endian implementation would produce `3f 00 00 00 78…` — a 4-byte shift that is visible by eye in a hex dump and is the canonical fingerprint of this bug. Little-endian is non-conforming.

**Check 1 — Signable byte assembly.** For each `HELLO` and `HEARTBEAT` vector, the implementation reconstructs the signable buffer from `inputs` and asserts byte-for-byte equality against `expected_signable_hex` (decoded bytes; see Hex comparison rule above). This catches endianness, UTF-8 byte-vs-character length, field-ordering, and `peerId` encoding bugs. (A signature mismatch alone does not localize the defect; the signable check does.)

**Check 2 — Ed25519 signature.** For each `HELLO` and `HEARTBEAT` vector, the implementation signs the decoded-bytes of `expected_signable_hex` with the test seed (`initiator_identity_seed_hex` or `responder_identity_seed_hex`) using its Ed25519 implementation, and asserts byte-for-byte equality against the decoded bytes of `expected_signature_hex`. Per RFC 8032 Ed25519 is fully deterministic — there is no nonce/IV randomness — so any RFC-conforming implementation produces identical signature bytes. Note: these vectors test the **signing path only**; verification-side behavior (malleability, small-subgroup acceptance, ZIP-215) is a separate conformance concern not addressed here.

**Check 3 — CONFIRM SHA-256 transcript hash.** For each `CONFIRM_TRANSCRIPT` vector, the implementation reconstructs the input buffer from `inputs` and asserts byte-for-byte equality against the decoded bytes of `expected_input_hex`, then computes SHA-256 and asserts byte-for-byte equality against the decoded bytes of `expected_sha256_hex`. Both assertions are required. Source of `invite_capabilities`: on the initiator side, `invite_capabilities` is the byte the initiator placed in its own INVITE frame; on the responder side, it is the byte received in the INVITE frame. A3 vectors use a consistent single value for both sides (post-relay assumption); a successful CONFIRM proves the two values agreed; a mismatch detects in-flight INVITE tampering (the A2 MitM mitigation).
