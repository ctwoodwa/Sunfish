...
**Date:** 2026-05-04
**Authors:** XO research session
**Council posture:** pre-merge canonical (per ADR 0069; cohort batting average 23-of-23 substrate amendments needed council fixes — auto-merge NOT enabled)
...
- `Rationale` is nullable; the issuance-time rationale is mirrored when present, but the post-application observable view does not require it (unlike the issuance-time `StandingOrder.Rationale` which is required for forensic review).
...
- **§A0.2 Positive-existence** — verified on origin/main:
    ...
    - `Sunfish.Foundation.Assets.Common.{ActorId, TenantId, AuditRecordId}` (`packages/foundation/Assets/Common/`). **Correction to parent ADR 0065 §A0.2:** the parent ADR cited `Sunfish.Foundation.Identity.ActorId` and `Sunfish.Foundation.MultiTenancy.TenantId`. The actual on-origin/main namespaces are `Sunfish.Foundation.Assets.Common.ActorId` (`packages/foundation/Assets/Common/ActorId.cs:4`) and `Sunfish.Foundation.Assets.Common.TenantId` (`packages/foundation/Assets/Common/TenantId.cs`). `Sunfish.Foundation.MultiTenancy` is a real namespace (`packages/foundation-multitenancy/`) but it carries `ITenantScoped`, `IMustHaveTenant`, `ITenantCatalog`, `TenantStatus` — not the `TenantId` value type. This amendment cites the correct namespaces. Council should verify and consider whether the parent ADR's §A0.2 needs a follow-on mechanical correction.
...
The §A0 self-audit is *necessary but not sufficient* — council remains canonical defense per the cohort batting average of 23-of-23 prior substrate amendments needing council fixes. Auto-merge is NOT enabled on the amendment PR.

### Council brief — pressure-test points

Pre-flagged for the upcoming Opus 4.7 + xhigh canonical council:

1. **Issued-vs-Applied semantic distinction (load-bearing).** The amendment introduces `StandingOrderApplied` distinct from the existing `StandingOrderIssued`. Is the distinction necessary, or does the existing `StandingOrderIssued` already cover the post-application observation? Council should verify against ADR 0066 §1.3's actual subscription needs (does the Helm widget care about issuance-time or projection-incorporated-time?), and against the multi-anchor / Bridge-fanout latency story. A "single event suffices" finding would simplify the amendment to just adding `IStandingOrderEventStream` over `StandingOrderIssued`-shaped events.
2. **`IStandingOrderEventStream` vs `IObservable<StandingOrderAppliedEvent>`.** The amendment picks the named-interface form per kernel-audit precedent, but the intake left both options open. Council should spot-check whether any newer cohort ADR (post-W#42) prefers the `IObservable` form — if so, switch.
3. **Structural-citation regression on parent ADR 0065 §A0.2.** This amendment flags that parent ADR cites `Sunfish.Foundation.Identity.ActorId` (does not exist) and `Sunfish.Foundation.MultiTenancy.TenantId` (`MultiTenancy` namespace exists but doesn't carry `TenantId`). Council should confirm the correction and decide whether to file a separate mechanical-fix PR for ADR 0065's §A0.2 text (the impl on origin/main is already correct; only the prose drifted).
4. **DI wiring race for early subscribers.** Subscribers that register **after** the `DefaultStandingOrderIssuer` has already published events miss the in-flight history. `ReplayAll()` covers this, but the contract should make the "subscribe-then-replay-then-subscribe-for-future" idiom explicit (kernel-audit's pattern) — or it can drift. Council should weight whether the contract needs an example or stays minimal.
5. **Concurrent `Publish` ordering under contention.** `InMemoryStandingOrderEventStream.Publish` mirrors the kernel-audit lock pattern, but the amendment does not specify ordering guarantees across concurrent issuers. Council should confirm whether "FIFO-by-Publish-call-order" is sufficient or whether a happens-before-by-`AppliedAt` ordering is required (the latter is harder; the former matches the prior art).

A green council on these five points clears the amendment for merge.
