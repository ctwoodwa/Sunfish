...
tags: sunfish,blazor,autocomplete,observable,data,new,collection
published: True
position: 30
...
To refresh the AutoComplete data when using [`OnRead`](slug:autocomplete-events#onread), call the `Rebind` method of the SunfishAutoComplete reference. This will fire the `OnRead` event and execute the business logic in the handler.
...
<SunfishButton OnClick="@RebindAutoComplete">Rebind the AutoComplete</SunfishButton>

<SunfishAutoComplete TItem="@String"
                     @ref="@AutoCompleteRef"
                     OnRead="@ReadItems"
...
@code{
    private SunfishAutoComplete<string> AutoCompleteRef { get; set; }
...
<SunfishButton OnClick="@AddSuggestion">Add suggestion</SunfishButton>
<br />

<h4>Remove the last suggestion</h4>
<SunfishButton OnClick="@RemoveSuggestion">Remove the last suggestion</SunfishButton>
<br />

<h4>Autocomplete suggestions: @Suggestions.Count</h4>
<br />

<SunfishAutoComplete Data="@Suggestions" ValueField="@( nameof(SuggestionsModel.Suggestion) )" @bind-Value="@TheValue" />
