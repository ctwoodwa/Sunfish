...
page_title: Sunfish UI for Blazor TreeView Documentation | TreeView Accessibility
description: "Get started with the Sunfish UI for Blazor TreeView and learn about its accessibility support for WAI-ARIA, Section 508, and WCAG 2.2."
...
Out of the box, the Sunfish UI for Blazor TreeView provides extensive accessibility support and enables users with disabilities to acquire complete control over its features.
...
If the tree supports the load-more functionality, the **Load More** button is rendered as the `li.mar-tree-item` element and has `role="button"`. The list item does not implement any of the following attributes.

| Selector | Attribute | Usage |
| -------- | --------- | ----- |
| `.mar-treeview` | `role=tree` | The root `div` element of the treeview. |
| `.mar-tree-item__children` | `role=group` | The `ul` element that wraps child nodes. |
| `.mar-treeview__filter-input` | `aria-label` | Specifies the announced name for the filter input. |
|  | `role=searchbox` | Specifies the role of the filter input. |
|  | `aria-controls=.mar-treeview id` | Points to the main ul element .mar-treeview. |
| `.mar-tree-item` | `role=treeitem` | The `li` element rendered for a tree node. |
...
| `.mar-tree-item:has(>.mar-tree-item--selected)` | `aria-selected=true` | Rendered only when selection is enabled. Announces the selected state of the node. |
| `.mar-tree-item__checkbox` | `role=none/presentation` | Added to the wrapper element of the checkbox to prevent duplicated information announced to the user. The checked state is controlled by `aria-checked`. |
...
For details on how the TreeView keyboard navigation works, refer to the [Blazor TreeView Accessibility and Keyboard Navigation Demo](https://demos.sunfish.dev/blazor-ui/treeview/keyboard-navigation).
