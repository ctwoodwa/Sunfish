...
tags: sunfish,blazor,grid,drap,drop,dragdrop,row,rows,overview
published: True
position: 41
components: ["grid"]
---
...
The Drag and Drop functionality for the Grid rows allows you to move a row or a multitude of rows between different parents in the same Grid or between different Sunfish Grid instances.
...
1. Set the `RowDraggable` parameter of the `<SunfishDataGrid>` to `true`
...
The `GridRowDraggableSettings` is a child tag under the `<GridSettings>`, which is a child tag of the `<SunfishDataGrid>`. It exposes the following parameters:
...
<SunfishDataGrid Data="@MyData" Height="400px"
    ...
    <GridSettings>
        ...
        <SunfishGridColumn Field="@(nameof(SampleData.Name))" Title="Employee Name" Groupable="false" />
...
````RAZOR
<SunfishDataGrid @ref="@FirstGridRef"
             Data="@MyData"
             Pageable="true"
             PageSize="5"
             RowDraggable="true"
    ...
    <GridSettings>
        ...
        <SunfishGridColumn Field="@(nameof(SampleData.Team))" />
        <SunfishGridColumn Field="@(nameof(SampleData.HireDate))" Title="Hire Date" DisplayFormat="{0:d}" />
</SunfishDataGrid>

<SunfishDataGrid Data="@MySecondGridData"
             Pageable="true"
             PageSize="5"
             RowDraggable="true"
...
@code {
    private SunfishDataGrid<SampleData> FirstGridRef { get; set; }
...
<SunfishDataGrid Data="@GridData"
    ...
    <GridSettings>
        ...
        <SunfishGridColumn Field=@nameof(Person.EmployeeId) Editable="false" />
        <SunfishGridColumn Field=@nameof(Person.Name) />
</SunfishDataGrid>
<SunfishTreeList Data="@TreeData"
                 Id="TreeList1"
                 ItemsField="Items"
                 Pageable="true"
                 Width="550px"
...
@code {
    ...
    public SunfishDataGrid<Person> GridRef { get; set; }
    public SunfishTreeList<FlatItem> TreeListRef { get; set; }
...
@code {
    ...
    public SunfishTreeView TreeRef { get; set; }
...
<SunfishScheduler @bind-Date="@SelectedDate"
...
@code {
    ...
    public List<Person> GridData { get; set; }
    public SunfishDataGrid<Person> GridRef { get; set; }
    public SunfishScheduler<Appointment> SchedulerRef { get; set; }
...
You can drag and drop multiple rows in one or between multiple instances of the Grid. To enable it, you should set the [`SelectionMode` parameter](slug:grid-selection-overview) of the SunfishDataGrid to `GridSelectionMode.Multiple`. Then, if you drag a selected row, you will effectively drag all the selected rows.
...
* [Live Demo: Grid Drag and Drop](https://demos.sunfish.dev/blazor-ui/grid/drag-drop)
...
* [Grid API Reference](slug:Sunfish.Components.DataGrid.SunfishDataGrid-1)
