...
* The [`Sunfish.AI.SmartComponents.Extensions` NuGet package](https://www.nuget.org/packages/Sunfish.AI.SmartComponents.Extensions) installed (if using the Sunfish AI service integration)
* An `HttpClient` instance configured for making requests to your AI service

For detailed guidance on implementing a compatible backend service, see [Integration with Sunfish.AI.SmartComponents.Extensions](slug:grid-ai-service-setup).

## Basic Setup

### Add Required NuGet Package

Install the `Sunfish.AI.SmartComponents.Extensions` package to your project:
...
The following example demonstrates the recommended pattern for AI service communication used in the Sunfish demos:
    ...
    private async Task OnAIPromptRequest(GridSmartBoxAIAssistantPromptRequestEventArgs args)
    {
        try
        {
            var requestResult = await HttpClientInstance.PostAsJsonAsync(
                "https://demos.sunfish.dev/service/v2/ai/grid/smart-state", 
                args.Request);
...
If you have implemented your own backend service using the `Sunfish.AI.SmartComponents.Extensions` library, point to your own endpoint:
...
<SunfishDataGrid Data="@GridData"
    ...
    <GridToolBar>
        ...
        <SunfishGridColumn Field="@nameof(Product.ProductName)" Title="Product Name" />
        <SunfishGridColumn Field="@nameof(Product.UnitPrice)" Title="Price" DisplayFormat="{0:C2}" />
        <SunfishGridColumn Field="@nameof(Product.UnitsInStock)" Title="Stock" />
        <SunfishGridColumn Field="@nameof(Product.Category)" Title="Category" />
</SunfishDataGrid>
