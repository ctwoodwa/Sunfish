...
tags: sunfish,blazor,grid,ai,assistant,toolbar
published: True
position: 20
components: ["grid"]
---
...
1. Add the [`Sunfish.AI.SmartComponents.Extensions` NuGet package](https://www.nuget.org/packages/Sunfish.AI.SmartComponents.Extensions) to your Blazor app.
1. Import the `Sunfish.AI.SmartComponents.Extensions` namespace in your `.razor` file or globally in `_Imports.razor`.
...
2. Subscribe to the `OnPromptRequest` event of the nested `<GridToolBarAIAssistantToolPromptSettings>` component. It is effectively the `OnPromptRequest` event of a [Sunfish AIPrompt component](slug:aiprompt-events).
...
The following online demos show complete implementations of the Grid AI smart functionality with the `GridToolBarAIAssistantTool`. These examples use a Sunfish-hosted AI service for demonstration purposes only. See [Integration with Sunfish.AI.SmartComponents.Extensions](slug:grid-ai-service-setup) for more information on how you can implement your own server-side AI service to be compatible with the Sunfish Grid.
...
* [Integration with Sunfish.AI.SmartComponents.Extensions](slug:grid-ai-service-setup)
