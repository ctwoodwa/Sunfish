...
tags: sunfish, blazor, grid, editing, popup
published: True
position: 2
components: ["grid"]
---
...
The popup edit mode can display a [Sunfish Validation Summary component](slug:validation-tools-summary) if the Grid model is configured for validation.
...
Starting with version 7.0, the Grid allows users to edit [hidden columns](slug:grid-columns-visible) by default. To disable editing of a hidden column, set `Editable="false"` to the respective `<SunfishGridColumn>` tag to make it read-only.
...
@[template](/_contentTemplates/grid/editing.md#advanced-example-toolbar)
        <SunfishGridColumn Field="@nameof(Product.Id)" Editable="false" Width="60px" />
        <SunfishGridColumn Field="@nameof(Product.Name)" />
        <SunfishGridColumn Field="@nameof(Product.Description)" EditorType="@GridEditorType.TextArea" Visible="false">
            <Template>
...
@[template](/_contentTemplates/grid/editing.md#advanced-example-columns)
        ...
        <SunfishGridCommandColumn Title="Commands" Width="180px">
            @{ var dataItem = (Product)context; }
...
* [Live Demo: Grid Popup Editing](https://demos.sunfish.dev/blazor-ui/grid/editing-popup)
