...
tags: sunfish,blazor,grid,export,csv
published: True
position: 3
components: ["grid"]
---
...
<SunfishDataGrid Data="@GridData"
    ...
    <GridExport>
        <GridCsvExport FileName="sunfish-grid-export" />
    </GridExport>

        <SunfishGridColumn Field="@nameof(SampleData.ProductId)" Title="ID" />
        <SunfishGridColumn Field="@nameof(SampleData.ProductName)" Title="Product Name" />
        <SunfishGridColumn Field="@nameof(SampleData.UnitsInStock)" Title="In stock" />
        <SunfishGridColumn Field="@nameof(SampleData.Price)" Title="Unit Price" />
        <SunfishGridColumn Field="@nameof(SampleData.Discontinued)" Title="Discontinued" />
        <SunfishGridColumn Field="@nameof(SampleData.FirstReleaseDate)" Title="Release Date" />
</SunfishDataGrid>
...
| --- | --- | --- |
| `SaveAsCsvFileAsync` | `ValueTask` | Sends the exported CSV file to the browser for download. You can pass [`GridCsvExportOptions`](slug:Sunfish.Components.DataGrid.SunfishDataGrid-1) to customize the export. |
| `ExportToCsvAsync` | `Task<MemoryStream>` | Returns the exported data as a `MemoryStream`. The stream itself is finalized, so that the resource does not leak. To read and work with the stream, clone its available binary data to a new `MemoryStream` instance. You can pass [`GridCsvExportOptions`](slug:Sunfish.Components.DataGrid.GridCsvExportOptions) to customize the export. |
...
@using System.IO
@using Sunfish.Components.DataGrid;

<SunfishButton OnClick="@(async () => await GridRef.SaveAsCsvFileAsync())">Download the CSV file</SunfishButton>
<SunfishButton OnClick="@GetTheDataAsAStream">Get the Exported Data as a MemoryStream</SunfishButton>
<SunfishButton OnClick="@(async () => await SaveAsCsvWithOptions())">Download CSV with Options</SunfishButton>
<SunfishButton OnClick="@(async () => await ExportToCsvWithOptions())">Get CSV Data with Options</SunfishButton>

<SunfishDataGrid @ref="@GridRef"
             Data="@GridData"
             Pageable="true"
             Sortable="true"
             Resizable="true"
    ...
    <GridToolBarTemplate>
        ...
        <label class="k-checkbox-label"><SunfishCheckBox @bind-Value="@ExportAllPages" />Export All Pages</label>
    </GridToolBarTemplate>

    <GridExport>
        <GridCsvExport FileName="sunfish-grid-export" AllPages="@ExportAllPages" />
    </GridExport>

        <SunfishGridColumn Field="@nameof(SampleData.ProductId)" Title="ID" Width="100px" />
        <SunfishGridColumn Field="@nameof(SampleData.ProductName)" Title="Product Name" Width="300px" />
        <SunfishGridColumn Field="@nameof(SampleData.UnitsInStock)" Title="In stock" Width="100px" />
        <SunfishGridColumn Field="@nameof(SampleData.Price)" Title="Unit Price" Width="200px" />
        <SunfishGridColumn Field="@nameof(SampleData.Discontinued)" Title="Discontinued" Width="100px" />
        <SunfishGridColumn Field="@nameof(SampleData.FirstReleaseDate)" Title="Release Date" Width="300px" />
</SunfishDataGrid>

@code {
    private SunfishDataGrid<SampleData> GridRef { get; set; }
...
* [Live Demo: Grid Export](https://demos.sunfish.dev/blazor-ui/grid/export)
