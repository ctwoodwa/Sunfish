...
tags: sunfish,blazor,grid,read,filter,sort,page,manual,data,data source
published: True
position: 55
components: ["grid"]
---
...
* [Sunfish .ToDataSourceResult(request)](#sunfish-todatasourceresultrequest)
...
* [Use OData Service](https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/odata)
* [Serialize the DataSoureRequest to the server](https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/datasourcerequest-on-server)
* [Debounce Data Source Operations and Requests](slug:grid-kb-debounce-operations)

## Custom paging with a remote service

>note The example below demonstrates how to use just Paging with a remote service. For a more complex setup including other Grid functionalities (such as sorting, filtering etc.) you can check [this project for using Sunfish DataSourceRequest and DataSourceResult on the server](https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/datasourcerequest-on-server) in our public repository.
...
<SunfishDataGrid TItem="@Employee"
			 OnRead="@ReadItems"
			 Pageable="true" PageSize="15">
		<SunfishGridColumn Field=@nameof(Employee.Id) Title="ID" />
		<SunfishGridColumn Field=@nameof(Employee.Name) Title="Name" />
</SunfishDataGrid>
...
@code {
	...
	//https://docs.sunfish.dev/blazor-ui/components/grid/editing/overview
...
## Sunfish .ToDataSourceResult(request)

If you have all the data at once, the Sunfish .ToDataSourceResult(request) extension method can manage the operations for you.

>tip You can find examples of how to use this object to easily retrieve data on the server in a performant manner in the following repo: [https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/datasourcerequest-on-server](https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/datasourcerequest-on-server).
>
> We support the `System.Text.Json` serialization that is built-in in Blazor.

>caption Use Sunfish .ToDataSourceResult() extension method to filter, sort and page data.

````RAZOR
Using Sunfish DataSource extension methods to manipulate all the data into paged chunks and also perform other operations like filtering, sorting, etc. There is a deliberate delay in the data source operations in this example to mimic real life delays and to showcase the async nature of the calls.

@using Sunfish.DataSource.Extensions

<SunfishDataGrid TItem="@Employee" OnRead="@ReadItems"
             FilterMode="@GridFilterMode.FilterRow"
             Sortable="true" Pageable="true">
        <SunfishGridColumn Field=@nameof(Employee.ID) />
        <SunfishGridColumn Field=@nameof(Employee.Name) Title="Name" />
        <SunfishGridColumn Field=@nameof(Employee.HireDate) Title="Hire Date" />
</SunfishDataGrid>
...
When the Grid needs to be grouped, the shape of the data changes and it is no longer a flat list of models. Instead, the data is a nested list of [`AggregateFunctionsGroup` objects](slug:sunfish.datasource.aggregatefunctionsgroup) that describe each group and include its data items.
...
````RAZOR
This sample shows how to set up the grid to use grouping with manual data source operations, and how to use the Sunfish DataSource extensions to prepare grouped data.
...
<SunfishDataGrid TItem="@Employee"
             ...
             Sortable="true"
             Pageable="true">
        <SunfishGridColumn Field=@nameof(Employee.Name) FieldType="@(typeof(string))" Groupable="false" />
        <SunfishGridColumn Field=@nameof(Employee.Team) FieldType="@(typeof(string))" Title="Team" />
        <SunfishGridColumn Field=@nameof(Employee.IsOnLeave) FieldType="@(typeof(bool))" Title="On Vacation" />
</SunfishDataGrid>
...
@code {
    ...
    protected async Task ReadItems(GridReadEventArgs args)
    {
        // in this example, we use the Sunfish extension methods to shape the data
        // you can, instead, call a service, read more in the following example projects
        // https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/datasourcerequest-on-server
        var datasourceResult = SourceData.ToDataSourceResult(args.Request);
...
<SunfishDataGrid TItem="@SampleData"
    ...
    <GridToolBarTemplate>
        ...
        <SunfishGridColumn Field="Id" />
        <SunfishGridColumn Field="Name" />
        <SunfishGridColumn Field="Team" />
        <SunfishGridColumn Field="HireDate" />
</SunfishDataGrid>
...
* [Live Demo: Manual Data Source Operations](https://demos.sunfish.dev/blazor-ui/grid/manual-operations)
* [Use OData Service](https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/odata)
* [Custom Server Operations](https://github.com/ctwoodwa/sunfish-archive/blazor-ui/tree/master/grid/datasourcerequest-on-server)
* [DataSourceRequest Object API](slug:Sunfish.DataSource.DataSourceRequest)
* [DataSourceResult Object API](slug:Sunfish.DataSource.DataSourceResult)
