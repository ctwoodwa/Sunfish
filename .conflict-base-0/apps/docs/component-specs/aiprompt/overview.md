...
tags: sunfish,blazor,aiprompt,ai,prompt
published: True
position: 0
components: ["aiprompt"]
---
...
The <a href = "https://sunfish.dev/blazor-ui/ai-prompt" target="_blank">Blazor AIPrompt component</a> helps you write input (prompt) instructing the Generative AI to produce the desired response. 
...
>caption Basic configuration of the Sunfish AIPrompt

````RAZOR
<SunfishAIPrompt OnPromptRequest="@HandlePromptRequest"
...
For a complete list of available parameters and methods, refer to the [AIPrompt API Reference](slug:Sunfish.Blazor.Components.SunfishAIPrompt).

To use component methods, define a reference to the AIPrompt instance with the `@ref` directive. For example:

````RAZOR
<SunfishAIPrompt @ref="@AIPromptRef" 
                 OnPromptRequest="@HandlePromptRequest"
                 PromptSuggestions="@Suggestions">
</SunfishAIPrompt>
<div style="margin-top: 2em;">
    <SunfishTextBox @bind-Value="@CustomPrompt"></SunfishTextBox>
    <SunfishButton OnClick="@ExternalGenerateHandler">Generate</SunfishButton>
</div>
...
To integrate a built-in SpeechToTextButton in the AIPrompt component, set `EnableSpeechToText="true"`. Optionally, you can use the `<AIPromptSettings>` tag as a child of `<SunfishAIPrompt>`. Inside `<AIPromptSettings>`, you can define the `<AIPromptSpeechToTextButtonSettings>` tag to configure the appearance and behavior of the built-in SpeechToTextButton. For a complete list of available parameters, refer to the [AIPromptSpeechToTextButtonSettings API Reference](slug:Sunfish.Blazor.Components.AIPromptSpeechToTextButtonSettings).
...
````RAZOR.skip-repl
<SunfishAIPrompt EnableSpeechToText="true" Prompt="@Prompt" PromptChanged="@OnPromptChanged">
    <AIPromptSettings>
...
* [AIPrompt API Reference](slug:Sunfish.Blazor.Components.SunfishAIPrompt)
