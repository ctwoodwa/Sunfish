...
tags: sunfish,blazor,listview,manual,operadtions,onread
published: True
position: 5
components: ["listview"]
---
...
<SunfishListView TItem="@SampleData"
...
<SunfishListView Data="@ListViewData" Pageable="true">
    <HeaderTemplate>
        Sort Name: <SunfishDropDownList Data="@( new List<string>() { "None", "Ascending", "Descending" } )"
                                        Value="@sort" ValueChanged="@( (string dir) => SortName(dir) )" />
        <br />
        Filter: <SunfishTextBox Value="@filter" ValueChanged="@( (string val) => FilterData(val) )" />
    </HeaderTemplate>
    <Template>
        <h6>@context.Name</h6>
    </Template>
...
>
> You can also use the Sunfish extension methods - the `.ToDataSourceResult()` that takes a `DataSourceRequest` argument over the full collection of data and add filer and sort descriptors to it. Examples of doing that are available in the Live Demos: [ListView Filtering](https://demos.sunfish.dev/blazor-ui/listview/filtering) and [ListView Sorting](https://demos.sunfish.dev/blazor-ui/listview/sorting)

## See Also

  * [Live Demo: ListView Filtering](https://demos.sunfish.dev/blazor-ui/listview/filtering)
  * [Live Demo: ListView Sorting](https://demos.sunfish.dev/blazor-ui/listview/sorting)
