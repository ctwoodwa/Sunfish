...
tags: sunfish,blazor,wizard,form,integration
published: True
position: 5
components: ["wizard"]
---
...
The Wizard component provides integration with the [Sunfish Form](slug:form-overview).

Each step of the Wizard can include an instance of Sunfish Form inside its `Content` tag. You will be able to use all the built-in features of the Form including its [Validation](slug:form-validation) to achieve the desired Wizard configuration.
...
>caption Integrate a Form component in the Sunfish Wizard. Use its validation state to control the `Valid` parameter of the Steps. The result from the snippet.
    ...
    <SunfishWizard @bind-Value="@Value" OnFinish="@OnFinishHandler" Width="700px">
        <WizardSteps>
                        ...
                        <FormItems>
                            ...
                            <FormItem Field="@nameof(User.Email)">
                                <Template>
                                    ...
                                    <SunfishTextBox Id="mail" @bind-Value="@UserModel.Email" InputMode="email" PlaceHolder="example@domain.com"></SunfishTextBox>
                                    <SunfishValidationMessage For="@(() => UserModel.Email)"></SunfishValidationMessage>
                                </Template>
                            </FormItem>
                            <FormItem Field="@nameof(User.Password)">
                                <Template>
                                    <label for="pass" class="k-label k-form-label">Password*:</label>
                                    <SunfishTextBox Id="pass" @bind-Value="@UserModel.Password" Password="true"></SunfishTextBox>
                                    <SunfishValidationMessage For="@(() => UserModel.Password)"></SunfishValidationMessage>
                                </Template>
                            </FormItem>
            ...
            <WizardStep Label="Shipping address" Icon="@SvgIcon.MapMarkerTarget" OnChange="@OnShippingStepChange" Valid="@IsShippingValid">
                <Content>
                    <SunfishForm Model="@ShippingModel"
                                 @ref="ShippingForm">
                        <FormValidation>
                            <DataAnnotationsValidator></DataAnnotationsValidator>
                        </FormValidation>
...
@code {
    ...
    public SunfishForm RegisterForm { get; set; }
    public User UserModel { get; set; } = new User();

    public SunfishForm ShippingForm { get; set; }
...
## See Also
  ...
  * [Live Demos: Wizard Form](https://demos.sunfish.dev/blazor-ui/wizard/form)