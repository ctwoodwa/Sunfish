...
description: Templated Item in the Sunfish ToolBar for Blazor
slug: toolbar-templated-item
tags: sunfish,blazor,toolbar,template,item,templateditem
published: True
position: 10
components: ["toolbar"]
---
...
The Sunfish ToolBar for Blazor allows you to add a custom element. To use it, add the `<ToolBarTemplateItem>` inside the `<SunfishToolBar>`. In it, you can add HTML and components according to your application needs.
...
>caption Add a Sunfish DropDownList to the ToolBar

![templated item for the ToolBar](images/toolbar-templated-item.png)

````RAZOR
@*Use the templated item to add a Sunfish DropDownList to the ToolBar*@
...
<SunfishToolBar>
    ...
    <ToolBarTemplateItem>
        <SunfishDropDownList Data="@Roles" Value="@SelectedRole" ValueChanged="@( (string v) => RoleChange(v) )"></SunfishDropDownList>
    </ToolBarTemplateItem>
</SunfishToolBar>
