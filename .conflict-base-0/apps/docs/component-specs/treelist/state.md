...
slug: treelist-state
tags: sunfish,blazor,treelist,state,save,load,layout,set,change,management
published: true
position: 50
components: ["treelist"]
---
...
The TreeList state is a generic [class `TreeListState<TItem>`](slug:Sunfish.Blazor.Components.TreeListState-1). The type depends on the type of the TreeList model. The `TreeListState<TItem>` object exposes the following properties:
...
* [Live Demo: TreeList State](https://demos.sunfish.dev/blazor-ui/treelist/persist-state)
* [TreeListState API reference](slug:Sunfish.Blazor.Components.TreeListState-1)
