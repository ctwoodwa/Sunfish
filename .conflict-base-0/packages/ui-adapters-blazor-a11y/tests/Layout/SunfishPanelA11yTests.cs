...
public class SunfishPanelA11yTests : IClassFixture<SunfishPanelA11yTests.Ctx>
{
    private readonly Ctx _ctx;

    public SunfishPanelA11yTests(Ctx ctx) => _ctx = ctx;

    [Fact]
    public async Task SunfishPanel_TitleAndContent_HasNoModeratePlusAxeViolations()
    {
        var rendered = _ctx.Bunit.Render<SunfishPanel>(p => p
        ...
        try
        {
            ...
            Assert.True(moderatePlus.Count == 0,
                $"SunfishPanel surfaced {moderatePlus.Count} moderate+ axe violation(s): " +
